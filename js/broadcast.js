	...
	content: function () {
		...
		game.broadcastAll(function (config) {
			if (lib.config.background_audio)
				game.playAudio("..", "extension", "星之梦/audio/effect", "bilibili_OpenTheGame" + (config === "default" ? "" : "_" + config));
...
lib.skill._bilibili_miaoshou = {
	...
	content: function () {
		...
		game.broadcastAll(function (config) {
			if (lib.config.background_audio)
				game.playAudio("..", "extension", "星之梦/audio/effect", "bilibili_miaoshou" + (config === "default" ? "" : "_" + config));
		}, config);
	},
};
lib.skill._bilibili_yishu = {
	...
	content: function () {
		...
		game.broadcastAll(function (config) {
			if (lib.config.background_audio)
				game.playAudio("..", "extension", "星之梦/audio/effect", "bilibili_yishu" + (config === "default" ? "" : "_" + config));
		}, config);
	},
};
...
lib.skill._bilibili_HighDamageAudio = {
	...
	content: function () {
		...
		if (trigger.num == 3) {
			...
			game.broadcastAll(function (config) {
				if (lib.config.background_audio)
					game.playAudio("..", "extension", "星之梦/audio/effect", "bilibili_diankuang" + (config ? "_decade" : ""));
			}, config);
		} else {
			player.$fullscreenpop(config ? "神威震乾坤" : "无双<br>万军取首", "fire");
			game.broadcastAll(function (config) {
				if (lib.config.background_audio)
					game.playAudio("..", "extension", "星之梦/audio/effect", "bilibili_wanjun" + (config ? "_decade" : ""));
			}, config);
		}
	},
};
...
lib.element.player.trySkillAnimate = function (name, popname, checkShow) {
	if (
		...
		lib.config.extension_星之梦_HDskillAnimateAudio
	) {
