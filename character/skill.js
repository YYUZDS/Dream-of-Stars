...
const lmCharacter = {
	...
	skill: {
			...
			content() {
				...
				switch (name) {
					case "sw_guanyu":
						{
							player
								.chooseButton(
									["请选择你的初始神武", [["sw_guilongzhanyuedao", "mj_guilongzhanyuedao", "mj_mengyanchitu"], "vcard"]],
									true
								)
					...
					case "sw_lvmeng":
						{
							player
								.chooseButton(["请选择你的初始神武", [["sw_guofengyupao", "mj_shengguangbaiyi"], "vcard"]], true)
								.set("ai", function (button) {
					...
					case "sw_zhugeliang":
						{
							player
								.chooseButton(["请选择你的初始神武", [["sw_qimenbagua", "mj_qixingpao"], "vcard"]], true)
								.set("ai", function (button) {
					...
					case "sw_zhouyu":
						{
							player
								.chooseButton(["请选择你的初始神武", [["sw_chiyanzhenhunqin", "mj_zhenhunqin"], "vcard"]], true)
								.set("ai", function (button) {
					...
					case "sw_lvbu":
						{
							player
								.chooseButton(["请选择你的初始神武", [["sw_xiuluolianyuji", "mj_xieshenmianju"], "vcard"]], true)
								.set("ai", function (button) {
					...
					case "sw_zhaoyun":
						{
							player
								.chooseButton(["请选择你的初始神武", [["sw_chixueqingfeng", "mj_yinyueqiang"], "vcard"]], true)
								.set("ai", function (button) {
			...
			subSkill: {
				damage: {
					...
					content() {
						...
						player
							.chooseTarget(
								[1, player.countMark("wechattianfa")],
								get.prompt("wechattianfa"),
								"对至多" + get.cnNumber(player.countMark("wechattianfa")) + "名其他角色造成1点伤害",
								lib.filter.notMe
							)
							.set("ai", function (target) {
		...
		old_mbqianlong: {
			...
			filter(event, player) {
				...
				let target =
					player.countMark("old_mbqianlong") == lib.skill.old_mbqianlong.maxMarkCount
						? lib.skill.old_mbqianlong.derivation.length
						: Math.floor(player.countMark("old_mbqianlong") / 25);
			...
			async content(event, trigger, player) {
				const derivation = lib.skill.old_mbqianlong.derivation,
					skills =
						player.countMark("old_mbqianlong") == lib.skill.old_mbqianlong.maxMarkCount
							? derivation
							: derivation.slice(0, Math.floor(player.countMark("old_mbqianlong") / 25));
					...
					.chooseButtonTarget({
						...
						ai1(button) {
							...
							if (
								!game.hasPlayer(
									current =>
										player != current && current.countDiscardableCards(player, "h") > 0 && get.attitude(player, current) < 0
								)
							) {
								return 0;
							}
			...
			async content(event, trigger, player) {
				...
				let result = await player
					...
					.set("filterButton", button => {
						...
						if (
							!button.links.length ||
							button.links.some(card => !lib.filter.cardDiscardable(card, player, get.event().getParent().name))
						) {
							return false;
						}
						return true;
		...
		old_guxuan: {
			...
			async cost(event, trigger, player) {
				...
				const result = await player
					...
					.set(
						"prompt",
						`###${get.prompt(event.skill)}###你可以弃置自己一个区域内的所有牌，然后选择弃置至多等量名其他角色对应区域内的各一张牌。`
					)
					.set("ai", (event, player) => {
				...
				while (num-- > 0) {
					...
					let result = await player
						...
						.set(
							"filterTarget",
							(_, player, target) =>
								target != player && !get.event().doneList.includes(target) && target.countDiscardableCards(player, get.event().pos)
						)
						.set("ai", target => {
		...
		oldx_juejin: {
			...
			async content(event, trigger, player) {
				...
				let result = await player
					.chooseButton(
						[1, num],
						[`###${get.prompt(event.name)}###执行${num == 1 ? "一" : `一至${get.cnNumber(num)}`}项`, [list, "textbutton"]]
					)
					.set("ai", ({ link }) => {
			...
			chooseButton: {
				...
				prompt(links) {
					return (
						...
						(_status.event.old_xinjianying_suit ? "(" + get.translation(_status.event.old_xinjianying_suit) + ")" : "") +
						"使用"
					);
				},
			},
		...
		old_sbanguo: {
			...
			subSkill: {
				...
				move: {
					...
					filter(event, player) {
						return (
							game.hasPlayer(current => current.hasSkill("old_sbanguo_mark")) &&
							game.hasPlayer(current => !current.hasMark("old_sbanguo_marked") && current != player)
						);
					},
					content() {
						"step 0";
					...
					content() {
						...
						if (hp > 0 && maxhp > 0) {
							player
								...
								.set("prompt", "安国：请选择一项")
								.set("choiceList", [
									"失去" + hp + "点体力，令" + get.translation(trigger.player) + "获得" + hp + "点护甲",
									"减" + maxhp + "点体力上限，令" + get.translation(trigger.player) + "获得" + maxhp + "点护甲",
								])
								.set("ai", () => "选项一");
		...
		old_mbjiwei: {
			...
			async content(event, trigger, player) {
				...
				if (trigger.name == "phase") await player.draw(num);
					...
					const control =
						colors.length == 1 ? colors[0] : await player.chooseControl(colors).set("prompt", "济危：请选择一个颜色").forResult();
		...
		old_jueyong: {
			...
			filter(event, player) {
				return (
					...
					event.getParent(2).name != "old_jueyong_timeout" &&
					get.position(event.cards[0], true) == "o" &&
					event.card.name == event.cards[0].name &&
					(!player.storage.old_jueyong || player.storage.old_jueyong[0].length < player.getHp())
				);
			},
			content() {
				...
				effect: {
					target_use(card, player, target, current) {
						if (
							get.type(card) == "equip" &&
							!get.tag(card, "gifts") &&
							target.storage.old_jueyong &&
							target.storage.old_jueyong[1].length
						) {
				...
				gain: {
					...
					filter(event, player) {
						...
						return game.hasPlayer(function (current) {
							...
							return event
								.getg(current)
								.some(card => get.suit(card, current) == player.storage.old_yichong && lib.filter.canBeGained(card, current, player));
						});
					},
					forced: true,
					content() {
						...
						var cards = trigger
							.getg(target)
							.filter(card => get.suit(card, target) == player.storage.old_yichong && lib.filter.canBeGained(card, target, player));
		...
		old_wufei: {
			...
			content() {
				...
				if (trigger.name == "damage") {
					player
						.chooseBool(get.prompt("old_wufei", target), "令" + get.translation(target) + "受到1点无来源伤害")
					...
					if (result.bool) {
						...
						var result2 = await player
							.choosePlayerCard(
								...
								"h",
								"visible",
								"<div class='text center'>选择其中至多三张牌，然后你可以将其中至多两张牌交给另一名其他角色，然后弃置剩余的牌</div>",
								[1, 3],
								true
							)
							...
							if (!game.hasPlayer(targetx => targetx != player && targetx != target)) result3 = { bool: false };
								...
								result3 = await player
									...
									.set("ai", button => {
										...
										if (
											!game.hasPlayer(
												target => target != player && target != _status.event.target && get.attitude(player, target) > 0
											)
										)
											return 0;
						...
						if (get.tag(card, "damage") && target.hasMark("luanchou")) {
							var other = game.findPlayer(function (current) {
								return (
									...
									(!current.storage.counttrigger || !current.storage.counttrigger.old_gonghuan)
								);
							});
							if (!other) {
			...
			content() {
				...
				if (game.countPlayer() == 2)
					...
					player
						...
						.set(
							"aiTargets",
							lib.skill.old_mbhuiyao.getUnrealDamageTargets(player, [game.filterPlayer(i => i != player), game.filterPlayer()], true)
						);
				("step 2");
				if (result.bool) {
					var targets = result.targets;
					...
					if (target.isHealthy()) event._result = { index: 0 };
						...
						target
							...
							.set("ai", () => {
								...
								return get.effect(player, { name: "draw" }, player, player) * len >= get.recoverEffect(player, player, player)
									? 0
									: 1;
							})
							.set("len", len);
		...
		old_mbcuizhen: {
			...
			async cost(event, trigger, player) {
				...
				event.result = await player
					.chooseTarget(
						get.prompt("old_mbcuizhen"),
						"废除至多" + get.cnNumber(num) + "名其他角色的武器栏",
						[1, num],
						(card, player, target) => {
		...
		old_sbliegong: {
			...
			filter(event, player) {
				return (
					!event.getParent()._old_sbliegong_player &&
					event.targets.length == 1 &&
					event.card.name == "sha" &&
					player.getStorage("old_sbliegong").length > 0
				);
			},
			prompt2(event, player) {
				let str = "",
			...
			subSkill: {
				...
				count: {
					...
					content() {
						...
						player.addTip(
							"old_sbliegong",
							get.translation("old_sbliegong") +
							player.getStorage("old_sbliegong").reduce((str, suit) => str + get.translation(suit), "")
						);
					},
				},
			},
		...
		old_sbzhiheng: {
			...
			prompt(event) {
				...
				var str =
					"出牌阶段限一次。你可以弃置任意张牌并摸等量的牌，若你以此法弃置的牌包括你所有手牌，则你多摸" + get.cnNumber(count + 1) + "张牌。";
				return str;
			},
			check(card) {
				var player = _status.event.player;
				...
				skillTagFilter(player, tag, arg) {
					if (tag === "nokeep")
						return (
							...
							player.isPhaseUsing() &&
							!player.getStat().skill.old_sbzhiheng &&
					...
					content() {
						...
						player
							...
							.set("ai", target => {
								var player = _status.event.player;
								return (
									-get.attitude(player, target) * target.getSkills().filter(skill => skill.indexOf("old_sbtianxiang_") == 0).length
								);
							});
						("step 1");
						if (result.bool) {
					...
					ai: {
						...
						skillTagFilter(player, tag, arg) {
							if (tag === "nokeep") {
								...
								return (
									player.isPhaseUsing() &&
									(player.getStat("skill").old_sbjiang_qiben || 0) < limit &&
									player.hasCard(card => get.name(card) != "tao", "h")
								);
							}
						},
				...
				effect: {
					target(card, player, target) {
						...
						if (
							...
							((target.hasZhuSkill("old_sbzhiba") && game.countPlayer(current => current != target && current.group == "wu")) ||
			...
			subSkill: {
				...
				mark: {
					...
					content() {
						...
						player.storage.old_sbhuoji_count = player
							.getAllHistory("sourceDamage", evt => evt.hasNature("fire") && evt.player != player)
							.reduce((num, evt) => num + evt.num, 0);
		...
		old_sbguanxing: {
			...
			filter(event, player) {
				return (
					...
					(player.hasSkill("old_sbguanxing_on") && player.countCards("s", card => card.hasGaintag("old_sbguanxing")))
				);
			},
			forced: true,
			locked: false,
			...
			subSkill: {
				...
				mark: {
					...
					content() {
						...
						player.storage.oldx_sbhuoji_count = player
							.getAllHistory("sourceDamage", evt => evt.hasNature("fire") && evt.player != player)
							.reduce((num, evt) => num + evt.num, 0);
		...
		old_sbjuxiang: {
			...
			filter(event, player) {
				return (
					...
					(!_status.old_sbjuxiang_nanman || _status.old_sbjuxiang_nanman.length)
				);
			},
			...
			content() {
				...
				player
					.chooseTarget(
						"请选择【巨象】的目标",
						"将游戏外的随机一张【南蛮入侵】交给一名角色（剩余" + get.cnNumber(_status.old_sbjuxiang_nanman.length) + "张）",
						true
					)
					.set("ai", target => {
						var player = _status.event.player;
		...
		old_sbwusheng: {
			...
			async content(event, trigger, player) {
				var result = await player
					.chooseTarget(
						get.prompt("old_sbwusheng"),
						"选择一名非主公的其他角色，本阶段对其使用【杀】无距离和次数限制，使用【杀】指定其为目标后摸一张牌，对其使用五张【杀】后不能对其使用【杀】",
						(card, player, target) => {
				...
				effect: {
					...
					filter(event, player) {
						...
						if (event.name == "useCard")
							return event.targets.some(target => typeof player.storage.old_sbwusheng_effect[target.playerid] == "number");
		...
		old_sbjizhu: {
			...
			subSkill: {
				effect: {
					...
					filter(event, player) {
						return (
							player.checkCooperationStatus(event.player, "old_sbjizhu") &&
							["old_sblongdan", "jdlongdan"].some(skill => player.hasSkill(skill, null, null, false))
						);
					},
					content() {
				...
				rob: {
					...
					async content(event, trigger, player) {
						...
						if (hs.length) {
							...
							player.storage.old_sbxuanhuo[target.playerid] = lib.skill.old_sbxuanhuo.getNum(
								target,
								"old_sbxuanhuo_rob",
								"old_sbxuanhuo_mark"
							);
				...
				effect: {
					player_use(card, player, target) {
						if (
							...
							target &&
							target.group == "qun" &&
							player.hasZhuSkill("old_sbxueyi") &&
							player.countSkill("old_sbxueyi") < 2
						)
							return [1, 0.6];
					},
				},
		...
		old_sbweimu: {
			...
			async content(event, trigger, player) {
				if (trigger.name == "useCardToTarget" || trigger.name == "addJudge")
					player.logSkill("old_sbweimu", null, null, null, [get.rand(1, 2)]);
			...
			subSkill: {
				attack: {
					...
					filter(event, player) {
						if (event.card.name !== "sha") return false;
						return (
							player.getStorage("old_sbxianzhen_attack").includes(event.target) &&
							event.target.isIn() &&
		...
		old_sbliyu: {
			...
			filter(event, player) {
				return (
					event.player != player && event?.card?.name == "sha" && event.player.countGainableCards(player, "hej") > 0 && event.player.isIn()
				);
			},
			async cost(event, trigger, player) {
				const target = trigger.player;
		...
		old_mbfeijing: {
			...
			async content(event, trigger, player) {
				const targets = event.targets.filter(target =>
					target.countCards("h", card => lib.filter.cardDiscardable(card, target, "old_mbfeijing"))
				);
				if (targets.length) {
					const next = player
				...
				gain: {
					...
					async cost(event, trigger, player) {
						...
						const { links, bool } = await player
							.chooseButton([
								"###" +
								get.prompt("old_friendxiaxing") +
								'###<div class="text center">移去1枚“启诲”标记，获得' +
								get.translation(gains) +
								"</div>",
								[storage.map(c => [c, get.translation(c)]), "tdnodes"],
							])
							.set("ai", button => {
								const player = get.player();
			...
			subSkill: {
								...
								game
									.getGlobalHistory(
										...
										evt => {
											...
											return (
												["lose", "loseAsync", "cardsDiscard"].includes(evt.name) && evt.getd().some(i => i.name === "sha")
											);
										},
										event
									)
				...
				lie: {
					...
					content() {
						...
						player
							.chooseUseTarget(
								"###" + get.prompt("old_potzhanlie") + '###<div class="text center">' + str + "视为使用一张无次数限制的【杀】</div>",
									...
									case "目标+1":
										player
											...
											.step(async (event, trigger, player) => {
												const result = await player
													.chooseTarget(
														"是否为" + get.translation(trigger.card) + "增加一个目标？",
														(card, player, target) => {
				...
				guanshi: {
					...
					filter(event, player) {
						...
						return (
							!event.getParent().directHit.includes(event.target) && player.getStorage("old_potzhanlie_guanshi").includes(event.card)
						);
					},
					forced: true,
					logTarget: "target",
		...
		old_potjiyu: {
			...
			subSkill: {
				...
				refresh: {
					...
					filter(event, player) {
						if (
							player.hasCard(card => card.hasGaintag("old_potjiyu_effect"), "h") ||
							typeof player.getStat("skill")?.["old_potjiyu"] !== "number"
						)
							return false;
						const evt = event.getl(player);
							...
							if (result?.bool) {
								...
								if (result.links?.length) {
									...
									if (owners.length <= 1) {
										...
										const cards = [
											player.getCards("he", i => result.links.includes(i)),
											target.getCards("he", i => result.links.includes(i)),
										];
										await game
											.loseAsync({
												lose_list: [
						...
						const result = await target
							...
							.set("ai", () => {
								...
								if (att > 0) {
									...
									return lib.suit
										.filter(suit => target.hasCard(card => get.suit(card, target) == suit, "h"))
										.reduce((min, current) =>
											target.countCards("h", { suit: current }) < target.countCards("h", { suit: min }) ? current : min
										);
								}
		...
		old_mbxishang: {
			...
			async cost(event, trigger, player) {
				...
				const createButton = (item, type, position, noclick, node) => {
					...
					node._customintro = [
						node => `形象：${lib.translate[node.skinSkill]}`,
						node => get.skillInfoTranslation(node.skinSkill, null, false),
					];
					return node;
				};
				const result = await player
		...
		oldx_clanlieshi: {
			...
			content() {
				...
				for (var i = 1; i <= 3; i++) {
					...
					else if (i == 2 && !target.countCards("h", { name: "shan" }))
						list[i - 1] = '<span style="opacity:0.5">' + list[i - 1] + "</span>";
			...
			ai: {
				...
				result: {
					player(player) {
						...
						if (
							choice[choice.length - 1] == "选项一" &&
							player.hp + player.countCards("hs", { name: ["tao", "jiu"] }) < 2 &&
							(player.identity == "zhu" || !player.hasFriend() || !player.hasSkill("bolhuanyin") || !player.countCards("h") >= 4)
						)
							return 0;
						if (
							game.hasPlayer(function (target) {
		...
		old_clandingan: {
			...
			async content(event, trigger, player) {
				...
				targets.sortBySeat();
				await game.asyncDraw(targets);
				const currents = targets.filter(
					target => target != player && target.isMaxHandcard(false, current => current != player && targets.includes(current))
				);
				if (!currents?.length) {
					return;
				}
		...
		old_clanyuzhi: {
			...
			filter(event, player, name) {
				...
				const num1 = player
					.getRoundHistory("gain", evt => evt.getParent().name == "draw" && evt.getParent(2).name == "old_clanyuzhi")
					.reduce((sum, evt) => sum + evt.cards.length, 0);
				const num2 = player
					.getRoundHistory("gain", evt => evt.getParent().name == "draw" && evt.getParent(2).name == "old_clanyuzhi", 1)
					.reduce((sum, evt) => sum + evt.cards.length, 0);
			...
			async content(event, trigger, player) {
				...
				const num1 = player
					.getRoundHistory(
						"gain",
						evt => evt.getParent().name == "draw" && evt.getParent(2).name == "old_clanyuzhi",
						name === "roundStart" ? 1 : 0
					)
					.reduce((sum, evt) => sum + evt.cards.length, 0);
				switch (name) {
					case "roundStart":
						const result = await player
							...
							.set("ai", function (card) {
								if (_status.event.dying && _status.event.num > 0 && get.cardNameLength(card) > _status.event.num)
						...
						let result2 = await player
							...
							.set("ai", function () {
								for (let i = 0; i < list.length; i++) {
									...
									if (get.plainText(translation).match(/“?出牌阶段限一次/g) || get.plainText(translation).match(/“?限定技/g))
										return i;
								}
								return 0;
							})
				...
				if (result.bool) {
					...
					if (target.name2 != undefined)
						target
							...
							.set("ai", function () {
								return lib.skill.old_dzgengzhan.getNum(target.name) < lib.skill.old_dzgengzhan.getNum(target.name2)
									? target.name
									: target.name2;
							});
			...
			subSkill: {
				jieming: {
					...
					content() {
						...
						player
							.chooseTarget(
								get.prompt("old_dzgengzhan"),
								"令一名手牌数小于其体力上限的角色摸三张牌，然后其将手牌弃至其体力上限",
								function (card, player, target) {
		...
		old_cihuang: {
			...
			getList(player, target, bool) {
				...
				for (var name of lib.inpile) {
					...
					if (
						...
						info.type != "trick" ||
						info.notarget ||
						(info.selectTarget && info.selectTarget != 1 && (info.selectTarget != -1 || !info.toself))
					)
						continue;
					tricks.push(name);
				}
			...
			content() {
				...
				player
					.chooseButton([
						get.prompt("old_cihuang", target),
						"<div class='text center'>选择一张牌并视为对" + get.translation(target) + "使用</div>",
						[list, "vcard"],
					])
					.set("ai", function (button) {
						var card = {
		...
		old_olchongshen: {
			...
			ai: {
				...
				skillTagFilter(player, tag, arg) {
					if (
						arg == "respond" ||
						!player.countCards("h", card => _status.connectMode || (card.hasGaintag("old_olchongshen") && get.color(card) == "red"))
					)
						return false;
				},
				effect: {
			...
			mod: {
				aiValue(player, card, num) {
					if (get.name(card) != "shan" && get.itemtype(card) == "card" && (!card.hasGaintag("old_olchongshen") || get.color(card) != "red"))
						return;
			...
			subSkill: {
				save: {
					...
					async content(event, trigger, player) {
						const target = event.targets[0];
						const { bool, links } = await player
							.chooseCardButton(get.translation("old_olchunlao"), player.getExpansions("old_olchunlao"), true)
							.forResult();
						if (bool) {
		...
		old_shajue: {
			...
			content() {
				...
				if (
					trigger.player.hp < 0 &&
					get.itemtype(trigger.parent.cards) == "cards" &&
					trigger.parent.cards.some(card => get.position(card, true) == "o")
				) {
					player.gain(
		...
		old_olguanbian: {
			...
			forced: true,
			async content(event, trigger, player) {
				if (event.triggername == "roundStart" || ["old_olxiongni", "old_olfengshang"].includes(trigger.name))
					await player.removeSkills(event.name);
			...
			async content(event, trigger, player) {
				...
				for (const target of event.targets) {
					const { bool } = await target
						.chooseToDiscard(
							`弃置一张${get.translation(suit)}牌，否则${get.translation(player)}对你造成1点伤害`,
							"he",
							(card, player) => {
		...
		old_olzhibin: {
			...
			filter(event, player) {
				...
				return (
					get.info("old_olzhibin").filterx(player, num) ||
					get.info("old_olzhibin").filtery(player, num) ||
					get.info("old_olzhibin").filterz(player, num)
				);
			},
			filterx(player, num) {
				return (
					...
					!game.getAllGlobalHistory(
						"everything",
						evt => evt.name == "gainMaxHp" && evt.player == player && evt.getParent().name == "old_olzhibin"
					).length
				);
			},
			filtery(player, num) {
				return (
					...
					!game.getAllGlobalHistory(
						...
						evt =>
							...
							evt.player == player &&
							evt.getParent().name == "old_olzhibin" &&
				...
				},
				{
					...
					effect: {
						...
						async cost(event, trigger, player) {
							...
							event.result = await player
								.chooseToDiscard(get.prompt2(name), "he", [1, Infinity], "chooseonly")
					...
					effect: {
						...
						check(event, player) {
							...
							return (
								...
								!target.hasSkillTag("filterDamage", null, { player: event.source, card: event.card })
							);
						},
						content() {
							lib.skill.old_olhedao.tianshuClear(event.name, player);
				...
				},
				{
					...
					effect: {
						...
						check(event, player) {
							...
							if (
								get.effect(event.source, { name: "draw" }, player, player) * 3 +
								event.num * get.damageEffect(player, event.source, player, event.nature) >
								0
							)
								return true;
				...
				effect: {
					...
					filter(event, player) {
						return (
							...
							event.targets.some(target => player.getStorage("olhedao_effect").includes(target))
						);
					},
					forced: true,
					popup: false,
					...
					await player
						.chooseButton(
							[
								...
								[ToItems.randomGets(3).map(item => [item, item.name]), "textbutton"],
							],
							true
						)
						.set("ai", () => 1 + Math.random())
					...
					const result = await player
						.chooseButton(
							[
								...
								[
									skills.map(item => [item, "（剩余" + player.storage[item][0] + "次）" + lib.translate[item + "_info"]]),
									"textbutton",
								],
							],
							true,
						...
						? await player
							.chooseButton(
								[
									...
									[skills.map(item => [item, get.translation(item + "_info")]), "textbutton"],
								],
								true
							)
							.set("ai", () => 1 + Math.random())
			...
			check: function (event, player) {
				...
				if (get.tag(event.card, "respondSha")) {
					...
					if (
						...
						(event.card.name === "zhujinqiyuan" && (event.card.yingbian || get.distance(event.player, player) < 0))
					)
						return true;
				}
				return false;
				...
				player
					...
					.filter(
						evt =>
							...
							player.hasHistory("sourceDamage", evtx => evtx.player != player) &&
							player.countCards("he")
					)
					.step(async () => {
					...
					filter(event, player) {
						return (
							...
							player.hasHistory("lose", evt => {
								return (
									evt.getParent() == event.getParent("useCard") &&
									Object.values(evt.gaintag_map).flat().includes("old_olqianxi_damage")
								);
							})
						);
					},
		...
		old_clanyangji: {
			...
			async content(event, trigger, player) {
				...
				while (
					!player.hasHistory("sourceDamage", evt => evt.getParent(4) === event) &&
					player.getCards("h").some(card => cards.includes(card) && player.hasUseTarget(card))
				) {
					const result = await player
				...
				if (
					lastCard &&
					get.suit(lastCard, player) == "spade" &&
					(!get.owner(lastCard) || get.position(lastCard) !== "h") &&
					target?.isIn() &&
					target.canAddJudge(get.autoViewAs({ name: "lebu" }, lastCard))
				) {
				...
				effect: {
					...
					async cost(event, trigger, player) {
						event.result = await player
							...
							.set(
								"filterTarget",
								(card, player, target) => target.hasCard(true, "h") && player.getStorage("old_clanjiewu_effect").includes(target)
							)
							.set("ai", target => {
								let items = target.getCards("h");
								let count = [...new Set(items.map(item => get.suit(item, target)))].length;
					...
					async content(event, trigger, player) {
						...
						const card = cards[0];
						await player
							.showCards(card, `${get.translation(player)}对${get.translation(target)}发动了【捷悟】`)
							.set("old_clanjiewu", true);
		...
		old_clangaoshi: {
			...
			prompt(event, player) {
				return (
					get.prompt("old_clangaoshi") +
					"（可亮出" +
					get.cnNumber(player.getHistory("useSkill", evt => ["tdojiewu", "tdojiewu_effect"].includes(evt.skill)).length) +
					"张牌）"
				);
			},
			async content(event, trigger, player) {
		...
		old_clanqingjue: {
			...
			filter(event, player) {
				...
				return (
					...
					player.countDiscardableCards(player, "h", card => !get.info("old_clanqingjue").isOnlySuit(card, player)) > 0
				);
			},
			async content(event, trigger, player) {
				const result = await player
		...
		old_clanxsyingxiang: {
			...
			async content(event, trigger, player) {
				...
				if ((trigger.relatedEvent || trigger.getParent()).name !== "useCard") {
					...
					if (
						!player.hasSkill("old_clanxsyingxiang_used") &&
						player.countDiscardableCards(player, "h", card => !get.info(skill).isOnlySuit(card, player)) > 0
					) {
		...
		old_dcsbchuanyu: {
			...
			subSkill: {
				give: {
					...
					filter(event, player) {
						return (
							lib.skill.old_dcsbchuanyu_give.getCards(event, player).length > 0 &&
							game.hasPlayer(target => !player.getStorage("old_dcsbchuanyu").includes(target))
						);
					},
					getCards(event, player) {
						const evt = event.getParent();
					...
					ai: {
						...
						skillTagFilter(player, tag, arg) {
							return (
								...
								arg.card &&
								arg.target &&
								arg.target.hasSkill("old_dcaichen") &&
				...
				next: {
					...
					filter(event, player) {
						...
						return (
							...
							event.player.canUse(event.card, player.getPrevious(), false)
						);
					},
					...
					ai: {
						effect: {
							player_use(card, player, target) {
								...
								if (sum < get.effect(player.getPrevious().getPrevious(), card, player, player.getPrevious()))
									return "zeroplayertarget";
							},
						},
					},
			...
			subSkill: {
				use: {
					...
					filter(event, player) {
						...
						var targets = event.targets.filter(
							target => target.getPrevious().hasSkill("old_congshi") || target.getNext().hasSkill("old_congshi")
						);
					...
					content() {
						...
						targets.forEach(
							target =>
							(trigger.card.old_congshi[target.playerid] = targetx.filter(
								current => current == target.getPrevious() || current == target.getNext()
							))
						);
					},
				},
		...
		old_rezhuhai: {
			...
			filter(event, player) {
				return (
					...
					event.player.isIn() &&
					(player.canUse("sha", event.player, false) || player.canUse("guohe", event.player))
				);
			},
			content() {
				"step 0";
		...
		old_dcporui: {
			...
			content() {
				...
				player.chooseCardTarget({
					...
					prompt2:
						"弃置一张基本牌并选择一名本回合失去过牌的非当前回合的其他角色，你视为对其依次使用" +
						get.cnNumber(Math.max(0, player.hp) + 1) +
						"张【杀】",
					filterCard(card) {
			...
			intro: {
				...
				content(storage, player) {
					...
					if (player.hasMark("old_dcgonghu_basic") && player.hasMark("old_dcgonghu_damage"))
						str += "已达成全部条件，使用红色基本牌不可被响应，红色锦囊牌可多指定一个目标。";
		...
		old_miaoxian: {
			hiddenCard(player, name) {
				return (
					get.type(name) == "trick" &&
					!player.getStorage("old_miaoxian_used").contains(name) &&
		...
		old_dcdehua: {
			...
			mod: {
				...
				cardEnabled(card, player) {
					if (
						player.getStorage("old_dcdehua").includes(card.name) &&
		...
		old_dcqingshi: {
			...
			content() {
				...
				var choiceList = [
					...
					"令任意名其他角色各摸一张牌",
					"摸" + get.translation(player.hp) + "张牌，然后【情势】于本回合失效",
				];
				...
				transfer: {
					...
					mod: {
						cardname(card, player) {
							...
							if (map && map[card.cardid] && get.itemtype(card) == "card" && card.hasGaintag("old_dcwuyou_transfer"))
		...
		old_dclianjie: {
			...
			subSkill: {
				used: {
					...
					intro: {
						content: (storage, player) =>
							`已摸点数：${get.translation(storage).replace("13", "K").replace("12", "Q").replace("11", "J").replace("1", "A")}`,
					},
				},
			},
		},
				...
				buff: {
					...
					check(event, player) {
						return (
							...
							(event.targets?.reduce((sum, i) => sum + get.effect(event.card, i, player, player), 0) < 0)
						);
					},
					trigger: { player: "useCard" },
					prompt2(event, player) {
						return (
							"进行一次判定，若判定结果为" +
							player.countMark("old_stardangchen_buff") +
							"的倍数，则" +
			...
			async content(event, trigger, player) {
				...
				if (name === "phaseBefore" || name === "enterGame") {
					...
					if (suit) {
						...
						player.addTip(
							"old_dcyitong",
							get.translation("old_dcyitong") + player.getStorage("old_dcyitong").reduce((str, suit) => str + get.translation(suit), "")
						);
					}
				} else {
					let gains = [];
		...
		old_dczhongtao: {
			...
			chooseButton: {
				dialog(event, player) {
					return ui.create.dialog(
						`###众讨###出牌阶段限一次，你可以选择至多${Math.min(4, player.getDamagedHp() + 2)}种花色，然后随机获得弃牌堆中你选择花色的各一张牌。`,
		...
		old_dcsbjunmou: {
			...
			async content(event, trigger, player) {
				...
				const result = await player
					.chooseCard(`隽谋：选择一张手牌，${bool ? "重铸此牌并横置一名角色" : "此牌视为无次数限制的火【杀】"}`, "h", true)
					.forResult();
				if (result?.cards?.length) {
					const card = result.cards[0];
					if (!bool) {
						...
						.set("customButton", button => {
							...
							if (target) {
								...
								div.innerHTML =
									...
									get.translation(get.suit(button.link, target)) +
									target.getName() +
									"</span>";
							}
						})
						...
						function (skill, targets, cards, id, player) {
							...
							for (let i = 0; i < targets.length; i++) {
								dialog.buttons[i].querySelector(".info").innerHTML =
									getName(targets[i]) + get.translation(cards[i].suit) + cards[i].number;
							}
						},
						event.skill,
						targets,
		...
		old_twyimou: {
			...
			content() {
				...
				var choiceList = [
					"令" + target + "获得牌堆里的一张【杀】",
					"令" + target + "将一张手牌交给另一名角色，然后" + target + "摸两张牌",
						...
						content(storage, player) {
							...
							if (player.hasMark("old_twchuanshu_mark")) {
								str +=
									...
									"外的角色造成伤害时，此伤害+" +
									player.countMark("old_twchuanshu_mark") +
									"；";
				...
				damage: {
					...
					filter(event, player, name) {
						...
						if (name == "damageBegin1")
							return event.card && event.card.old_twchuanshu_mark && !player.getStorage("old_twchuanshu_effect").includes(event.player);
						return (
			...
			async content(event, trigger, player) {
				...
				const { control } = await player
					...
					.set("ai", () => {
						...
						return player.hp + player.countCards("hs", card => player.canSaveCard(card, player)) - num2 > 0 && num > num2
							? "背水！"
							: "回血";
					})
					.set("num", num)
			...
			subSkill: {
				hurt: {
					...
					filter(event, player) {
						...
						return (
							(player == event.player && !player.getStorage("old_twyanshi").includes(event.source)) ||
							(player != event.source && player.getStorage("old_twyanshi").includes(event.player))
						);
					},
					content() {
		...
		old_twdanfa: {
			...
			filter(event, player) {
				return (
					...
					!player.getExpansions("old_twdanfa").some(card => get.suit(card) == get.suit(event.card))
				);
			},
			async content(event, trigger, player) {
				await player
								...
								switch (num) {
									...
									case "1": {
										if (
											game.hasPlayer(
												current =>
													(get.realAttitude || get.attitude)(player, current) < 0 && get.distance(player, current) > 1
											) &&
											!bool
										)
											return 0;
				...
				equip: {
					...
					mod: {
						globalFrom(from, to, distance) {
							return (
								distance +
								from.getStorage("old_twhuangzhu_equip").reduce((sum, name) => sum + (lib.card[name[2]]?.distance?.globalFrom || 0), 0)
							);
						},
						globalTo(from, to, distance) {
							return (
								distance +
								to.getStorage("old_twhuangzhu_equip").reduce((sum, name) => sum + (lib.card[name[2]]?.distance?.globalTo || 0), 0)
							);
						},
						attackRange(from, distance) {
							return (
								distance -
								from.getStorage("old_twhuangzhu_equip").reduce((sum, name) => sum + (lib.card[name[2]]?.distance?.attackFrom || 0), 0)
							);
						},
						attackTo(from, to, distance) {
							return (
								distance +
								to.getStorage("old_twhuangzhu_equip").reduce((sum, name) => sum + (lib.card[name[2]]?.distance?.attackTo || 0), 0)
							);
						},
					},
					onremove(player, skill) {
		...
		old_twpiankuang: {
			audio: "twpiankuang",
			getNum: (event, player) =>
				Math.min(3, player.getHistory("sourceDamage", evt => evt.card && evt.card.name == event.card.name && evt.card != event.card).length),
			trigger: {
				source: "damageBegin1",
			},
			filter(event, player) {
		...
		old_rexianyuan: {
			...
			subSkill: {
				give: {
					...
					filter(event, player) {
						return (
							player.hasMark("old_rexianyuan") &&
							game.hasPlayer(i => lib.skill.old_rexianyuan.subSkill.give.filterTarget(null, player, i))
						);
					},
					filterTarget(card, player, target) {
	...
	translate: {
		...
		old_juejin: "绝烬",
		old_juejin_info:
			"持恒技，限定技，出牌阶段或当你进入濒死状态时，你可以选择X项执行：①回复X点体力；②摸X张牌；③手牌上限+X；④对一名其他角色造成X点火焰伤害（X为你脱离濒死状态的次数，至多为4，至少为1）。然后你可以发动一次“回天”。",
		old_huitian: "回天",
		old_huitian_info:
			"每轮限一次，结束阶段，若你本回合使用【杀】造成过伤害且当前体力值小于等于2，你可以获得一个额外回合且你于该额外回合内：①摸牌阶段的摸牌数+2；②使用【杀】无次数限制且【杀】造成的伤害+1.每轮开始时，若你发动“回天”的次数大于2，你死亡。",
		old_xingzhen: "星阵",
		old_xingzhen_info:
			"出牌阶段限一次，你可以观看牌堆顶的Y张牌（Y为你的体力上限且至多为7），你可以用手牌中的任意张牌与其中的任意张牌交换；然后选择一名其他角色角色，令其选择一项：①展示牌堆顶Y张牌；②展示你的手牌。展示后，你可以将展示牌中的【杀】依次对其使用。",
		oldx_mb_shen_jiangwei: "旧手杀神姜维", //二版
		oldx_mb_shen_jiangwei_prefix: "旧|手杀神",
		oldx_juejin: "绝烬",
		oldx_juejin_info:
			"持恒技，限定技，出牌阶段或当你进入濒死状态时，你可以选择X项执行：①回复X点体力；②摸X张牌；③体力上限+X；④对一名其他角色造成X点雷电伤害（X为你脱离濒死状态的次数，至多为4，至少为1）。然后你可以发动一次“回天”。若你未使用此技能执行过选项，则你进入濒死状态后减一点体力上限并将体力回复至一点。",
		oldx_xingzhen: "星阵",
		oldx_xingzhen_info:
			"牌堆顶的7张牌始终对你可见，你的回合外，你可以如手牌般使用或打出其中的基本牌，你的回合内，你可以如手牌般使用或打出其中的锦囊牌。",
		old_mb_caomao: "旧手杀曹髦",
		...
		old_jilin: "戢鳞",
		old_jilin_info:
			"①游戏开始时，你将牌堆顶三张牌暗置于你的武将牌上，称为“志”。②当你成为其他角色使用牌的目标时，你可以明置一张暗置的“志”令此牌对你无效。③回合开始时，你可用任意张手牌替换等量暗置的“志”。",
		old_yingyou: "英猷",
		old_yingyou_info:
			"①出牌阶段开始时，你可明置一张“志”然后摸X张牌（X为明置的“志”的数量）。②当你失去与明置的“志”其中一张花色相同的牌时，你摸一张牌。",
		old_yingtian: "应天",
		...
		old_xinjianying: "渐营",
		old_xinjianying_info:
			"①当你于出牌阶段内使用与此阶段你使用的上一张牌点数或花色相同的牌时，你可以摸一张牌。②出牌阶段限一次，你可以将一张牌当做任意基本牌使用（不计入次数限制）。若你于此阶段内使用的上一张牌有花色，则此牌的花色视为上一张牌的花色。",
		xinshibei: "矢北",
		...
		old_sbanguo: "安国",
		old_sbanguo_info:
			"①游戏开始时，你令一名其他角色获得1枚“安国”标记（有“安国”的角色手牌上限基数等于体力上限）。②出牌阶段开始时，你可以将一名有“安国”的角色的所有“安国”移动给一名本局游戏未获得过“安国”的其他角色。③当你受到伤害时，若有有“安国”的角色且伤害值不小于你的体力值且此伤害没有来源或来源没有“安国”，防止此伤害。④一名角色进入濒死状态时，若其有你因〖安国①〗获得的“安国”，你移去其该“安国”，令其将体力回复至1点。然后你选择一项：1.若你的体力值大于1，你失去体力至1点；2.若你的体力上限大于1，你将体力上限减至1。最后你令其获得X点护甲（X为你以此法失去的体力值或减少的体力上限）。",
		old_mb_sp_zhenji: "旧手杀SP甄宓",
		old_mb_sp_zhenji_prefix: "旧|手杀SP",
		old_mbbojian: "博鉴",
		old_mbbojian_info:
			"锁定技。出牌阶段结束时，若你本阶段使用牌数与花色数与上个出牌阶段皆不同，你摸两张牌；否则你将一张弃牌堆中本阶段你因使用而失去的牌交给一名角色。",
		old_mbjiwei: "济危",
		old_mbjiwei_info:
			"锁定技。①每个其他角色的回合结束时，此回合每满足一项，你便摸一张牌：1.有至少一名角色失去过牌；2.有至少一名角色受到过伤害。②准备阶段，若你的手牌数不小于存活人数且不小于体力值，你须将手牌中数量较多颜色的牌全部分配给其他角色（若数量相同则选择一种颜色）。",
		old_fuqian: "旧手杀傅佥",
		old_fuqian_prefix: "旧|手杀",
		old_jueyong: "绝勇",
		old_jueyong_info:
			"锁定技。①当你不因【绝勇】成为唯一牌的目标时，若此牌不为转化牌且对应的实体牌牌数为1且不为【桃】或【酒】，则你将此牌置于你的武将牌上，称为“绝”，且取消此牌的目标。②结束阶段开始时，若你有“绝”，且“绝”的数量不小于你已损失的体力值，则你令所有“绝”的原使用者依次对你使用所有“绝”，将无法使用的“绝”置入弃牌堆。",
		old_poxiang: "破降",
		...
		old_yichong: "易宠",
		old_yichong_info:
			"①准备阶段，你可以选择一名其他角色并选择一个花色，然后你获得其所有此花色的牌，移除场上的所有“雀”标记，令其获得“雀”标记直到你的下个回合开始。②拥有“雀”标记的角色获得你最后一次发动〖易宠①〗选择的花色的牌后，你获得这些牌（你至多通过每个“雀”得到五张牌）。",
		old_wufei: "诬诽",
		old_wufei_info:
			"若场上存在拥有“雀”标记的角色A，则：①当你使用【杀】或伤害类锦囊牌指定第一个目标后，你令A成为此牌伤害来源。②当你受到伤害后，若A的体力值大于1且A的体力值大于你，则你可以对A造成1点伤害。",
		old_mb_chengui: "旧手杀陈珪",
		old_mb_chengui_prefix: "旧|手杀",
		old_guimou: "诡谋",
		old_guimou_info:
			"锁定技。游戏开始时/回合结束时，你随机/须选择以下一项直到你的下个准备阶段：①记录场上期间角色使用牌数；②记录期间场上角色弃置牌数；③记录期间场上角色获得牌数。准备阶段，你可以选择一名场上对应记录数值最少的其他角色，观看其手牌并选择其中至多三张牌，然后你可以将其中至多两张牌交给另一名其他角色，然后弃置其余牌。",
		old_yangfu: "旧杨阜",
		old_yangfu_prefix: "旧",
		oldx_jiebing: "借兵",
		...
		old_liubing: "流兵",
		old_liubing_info:
			"锁定技。①当你声明使用【杀】后，若此牌是你本回合使用的第一张有唯一对应实体牌的【杀】，则你将此牌的花色改为♦。②其他角色于其出牌阶段内使用的非转化黑色杀结算结束后，若此【杀】未造成伤害，则你获得之。",
		old_sp_xinpi: "旧信辛毗",
		...
		old_gonghuan: "共患",
		old_gonghuan_info:
			"锁定技。每回合限一次，一名其他角色受到伤害时，若其拥有“姻”标记且其体力值小于你，则你将伤害转移给自己。此伤害结算结束后，若你与其体力值相等，则你与其移去“姻”标记。",
		old_sp_lvfan: "旧严吕范",
		...
		old_mbquesong: "雀颂",
		old_mbquesong_info:
			"一名角色的回合结束阶段，若你于本回合内受到过伤害，你令一名角色选择一项：1.摸X张牌并复原武将牌（X为5-其装备区牌数，且至少为1）；2.回复一点体力。",
		old_shenpei: "旧审配",
		...
		old_mbcuizhen: "摧阵",
		old_mbcuizhen_info:
			"①游戏开始时，你可以废除至多三名其他角色的武器栏。②当你于出牌阶段使用伤害类牌指定其他角色为目标后，若目标角色的手牌数不小于体力值，你可以废除其武器栏。③摸牌阶段，你令额定摸牌数+X（X为所有角色被废除的武器栏数之和+1，至多为4）。",
		old_mb_wangjing: "旧王经",
		old_mb_wangjing_prefix: "旧",
		old_mbjiejian: "节谏",
		old_mbjiejian_info:
			"准备阶段，你可将任意张手牌交给任意名其他角色，并令这些角色获得“节谏”标记。“节谏”角色成为一张非你使用的非装备牌的唯一目标时，你可将此牌转移给你，然后摸一张牌。“节谏”角色的回合结束时，移去其“节谏”标记，若其体力值不小于X（X为你交给其牌时其的体力值），你摸两张牌。",
		...
		old_spdaoshu_info_identity:
			"每轮限一次。一名其他角色的出牌阶段开始时，若其有手牌，则你可以令其视为使用一张【酒】。其须声明一个基本牌的牌名，然后你判断其手牌区内是否有该牌名的牌。若你判断正确，则你随机获得其五张手牌，否则你不能响应其使用的牌直到回合结束。",
		old_spdaoshu_info_guozhan:
			"每轮限一次。一名其他角色的出牌阶段开始时，若其有手牌，则你可以令其视为使用一张【酒】。其须声明一个基本牌的牌名，然后你判断其手牌区内是否有该牌名的牌。若你判断正确，则你随机获得其五张手牌，否则你不能响应其使用的牌直到回合结束。",
		old_liwei: "旧李遗",
		old_liwei_prefix: "旧",
		old_jiaohua: "教化",
		...
		old_jiaohua_info:
			"出牌阶段限四次，你可以选择一个未被〖教化〗记录过的牌的类型，令一名角色从牌堆中获得一张此类型的牌，然后记录此类型，若基本、锦囊、装备均已被你发动〖教化〗记录，则你清空〖教化〗记录。",
		old_zhangbu: "旧张布",
		old_zhangbu_prefix: "旧",
		old_mbchengxiong: "惩凶",
		old_mbchengxiong_info:
			"你使用锦囊牌指定其他角色为目标后，可选择一名牌数不小于X的角色，弃置其一张牌（X为你此阶段使用的牌数）。若此牌颜色与你使用的锦囊牌相同，你对其造成1点伤害。",
		old_mbwangzhuang: "妄专",
		...
		old_mbzhuguo: "助国",
		old_mbzhuguo_info:
			"出牌阶段限一次，你可令一名角色将手牌调整至体力上限。然后：若其没有摸牌，则回复1点体力；若其因此成为手牌数最多的角色，你可以选另一名角色，令其选择是否对你指定的另一名角色使用一张无距离限制的【杀】。",
		old_mbanda: "谙达",
		...
		old_sbliegong: "烈弓",
		old_sbliegong_info:
			"①你使用【杀】可以选择你距离不大于此【杀】点数的角色为目标。②当你使用牌时，或成为其他角色使用牌的目标后，你记录此牌的花色。③当你使用【杀】指定唯一目标后，若你〖烈弓②〗的记录不为空，则你可亮出牌堆顶的X张牌（X为你〖烈弓②〗记录过的花色数-1），令此【杀】的伤害值基数+Y（Y为亮出牌中被〖烈弓②〗记录过花色的牌的数量），且目标角色不能使用〖烈弓②〗记录过花色的牌响应此【杀】。此【杀】使用结算结束后，你清除〖烈弓②〗的记录。",
		...
		old_sbtongye: "统业",
		old_sbtongye_info:
			"锁定技。结束阶段，你猜测场上装备牌数与你下一个准备阶段的场上装备牌数是否相等，并获得以下效果：你下一个准备阶段，若你猜对且“业”数小于4，你获得1枚“业”；若你猜错，你弃1枚“业”。",
		...
		old_sbqixi: "奇袭",
		old_sbqixi_info:
			"你可以将一张黑色牌当作【过河拆桥】使用。你使用非转化非虚拟的【过河拆桥】可以改为：出牌阶段，对一名区域内有牌的角色使用，你弃置其区域内的所有牌。",
		old_sbfenwei: "奋威",
		old_sbfenwei_info:
			"限定技，当一名角色使用的锦囊牌指定了至少两名角色为目标时，你可以令此牌对其中任意名角色无效，然后你获得牌堆中的X张【过河拆桥】（X为你选择的角色数且X至多为4）。",
		old_sb_xiaoqiao: "旧谋小乔",
		old_sb_xiaoqiao_prefix: "旧|谋",
		old_sbtianxiang: "天香",
		old_sbtianxiang_info:
			"①出牌阶段限三次，你可以交给一名没有“天香”标记的其他角色一张红色牌，然后令其获得此牌花色的“天香”标记。②当你受到伤害时，你可以移去一名角色的“天香”标记，若此“天香”标记为：红桃，你防止此伤害，其受到伤害来源对其造成的1点伤害（若没有伤害来源则改为无来源伤害）；方片，其交给你两张牌。③准备阶段，你移去场上所有的“天香”标记，然后摸X张牌（X为移去的“天香”标记数+3）。",
		old_sb_daqiao: "旧谋大乔",
		...
		old_sbzhiba: "制霸",
		old_sbzhiba_info:
			"主公技，限定技。当你进入濒死状态时，你可以回复X点体力并修改〖激昂③〗为“出牌阶段限X次”（X为场上吴势力角色数）。然后其他吴势力角色依次受到1点无来源伤害，且当有角色因此死亡后，你摸三张牌。",
		...
		old_sbkanpo: "看破",
		old_sbkanpo_info:
			"①一轮游戏开始时，你清除〖看破①〗记录的牌名，然后你可以依次记录共计三个未于本次清除过的非装备牌牌名（对其他角色不可见）。②当其他角色使用你〖看破①〗记录过的牌名的牌时，你可以移去一个〖看破①〗中的此牌名的记录，令此牌无效。",
		old_sb_zhugeliang: "牢谋诸葛亮",
		old_sb_zhugeliang_prefix: "牢|谋",
		old_sbguanxing: "观星",
		old_sbguanxing_info:
			"①准备阶段，你将所有“星”置入弃牌堆，将牌堆顶的X张牌置于你的武将牌上，称为“星”。然后你可以将任意张“星”置于牌堆顶（X为你此次移去的“星”数+1且至多为7，若你此前未发动过〖观星①〗则X为7）。②结束阶段，若你未于本回合的准备阶段将“星”置于过牌堆顶，你可以将任意张“星”置于牌堆顶。③你可以如手牌般使用或打出“星”。",
		old_sbkongcheng: "空城",
		old_sbkongcheng_info:
			"锁定技。当你受到伤害时，若你有〖观星〗，且若你：有“星”，你判定，若结果点数不大于你的“星”数，此伤害-1；没有“星”，此伤害+1。",
		...
		oldx_sbkanpo: "看破",
		oldx_sbkanpo_info:
			"①一轮游戏开始时，你清除〖看破①〗记录的牌名，然后你可以依次记录任意个未于上次发动〖看破①〗记录清除过的非装备牌牌名（对其他角色不可见，每局游戏至多记录10个牌名）。②其他角色使用你〖看破①〗记录过的牌名的牌时，你可以移去一个〖看破①〗中的此牌名的记录令此牌无效，然后你摸一张牌。",
		oldx_sb_zhugeliang: "旧谋诸葛亮",
		oldx_sb_zhugeliang_prefix: "旧|谋",
		oldx_sbguanxing: "观星",
		oldx_sbguanxing_info:
			"①准备阶段，你将所有“星”置入弃牌堆，将牌堆顶的X张牌置于你的武将牌上，称为“星”（X为7-此前发动〖观星①〗次数的两倍，且X至少为0）。然后你可以将任意张“星”置于牌堆顶。②结束阶段，若你未于本回合的准备阶段将“星”置于过牌堆顶，你可以将任意张“星”置于牌堆顶。③你可以如手牌般使用或打出“星”。",
		oldx_sbkongcheng: "空城",
		oldx_sbkongcheng_info:
			"锁定技。当你受到伤害时，若你拥有技能〖观星〗，且若你：有“星”，你判定，若结果点数不大于你的“星”数，此伤害-1；没有“星”，此伤害+1。",
		old_sb_menghuo: "旧谋孟获",
		old_sb_menghuo_prefix: "旧|谋",
		old_sbzaiqi: "再起",
		old_sbzaiqi_info:
			"蓄力技（3/7）。①弃牌阶段结束时，你可以消耗任意点蓄力值并选择等量名角色，然后令这些角色选择一项：1.令你摸一张牌；2.弃置一张牌，然后你回复1点体力。②每回合限一次。当你造成伤害后，你获得1点蓄力值。",
		old_sb_zhurong: "旧谋祝融",
		...
		old_sbjuxiang: "巨象",
		old_sbjuxiang_info:
			"锁定技。①【南蛮入侵】对你无效。②当其他角色使用【南蛮入侵】结算结束后，你获得此牌对应的所有实体牌。③结束阶段，若你未于本回合使用过【南蛮入侵】，你将一张游戏外的随机【南蛮入侵】（共八张）交给一名角色。",
		old_sb_guanyu: "旧谋关羽",
		old_sb_guanyu_prefix: "旧|谋",
		old_sbwusheng: "武圣",
		old_sbwusheng_wusheng_backup: "武圣",
		old_sbwusheng_info:
			"你可以将一张手牌当作任意【杀】使用或打出。出牌阶段开始时，你可以选择一名非主公的其他角色，本阶段对其使用【杀】无距离和次数限制，使用【杀】指定其为目标后摸一张牌，对其使用五张【杀】后不能对其使用【杀】。",
		old_sb_zhaoyun: "旧谋赵云",
		old_sb_zhaoyun_prefix: "旧|谋",
		old_sblongdan: "龙胆",
		old_sblongdan_info:
			"蓄力技（1/4）。①你可以消耗1点蓄力值，将【杀】当做【闪】或将【闪】当做【杀】使用或打出，然后摸一张牌。②一名角色的回合结束时，你获得1点蓄力值。",
		old_sbjizhu: "积著",
		...
		old_sblongdan_shabi_info:
			"蓄力技（1/4）。①你可以消耗1点蓄力值，将一张基本牌当做任意基本牌使用或打出，然后摸一张牌。②一名角色的回合结束时，你获得1点蓄力值。",
		old_sb_fazheng: "旧谋法正",
		...
		old_sbenyuan: "恩怨",
		old_sbenyuan_info:
			"锁定技。准备阶段，若场上存在有“眩”的角色，你移去该角色的“眩”，且你于其本次获得“眩”的期间内得到其的牌数：不小于3，你交给其两张牌；小于3，其失去1点体力，你回复1点体力。",
		old_sb_xuhuang: "旧谋徐晃",
		old_sb_xuhuang_prefix: "旧|谋",
		old_sbduanliang: "断粮",
		old_sbduanliang_info:
			"出牌阶段限两次。你可以与一名其他角色进行谋弈。若你赢，且你选择的选项为：“围城断粮”，若其判定区没有【兵粮寸断】，你将牌堆顶牌当【兵粮寸断】对其使用，否则你获得其一张牌；“擂鼓进军”，你视为对其使用一张【决斗】。",
		...
		old_sbwansha: "完杀",
		old_sbwansha_info:
			"①你的回合内，不处于濒死状态的其他角色不能使用【桃】。②一名角色进入濒死状态时，你可以观看其手牌并选择其中零至两张牌（其他人不可见），然后其选择一项：1.你将这些牌分配给任意名不为其的角色；2.其弃置除这些牌以外的牌。",
		old_sbwansha_rewrite: "完杀·改",
		old_sbwansha_rewrite_info:
			"①你的回合内，不处于濒死状态的其他角色不能使用【桃】。②一名角色进入濒死状态时，你可以观看其手牌并选择其区域内零至三张牌（其他人不可见），然后其选择一项：1.你将这些牌分配给任意名不为其的角色；2.其弃置除这些牌以外的牌。",
		old_sbluanwu: "乱武",
		old_sbluanwu_info:
		...
		old_sbweimu_rewrite_info:
			"锁定技。①当你成为黑色锦囊牌的目标时，取消之。②每轮开始时，若你上一轮成为其他角色使用牌的目标的次数不大于2，你从弃牌堆中随机获得一张黑色锦囊牌或防具牌。",
		...
		old_sbhongyuan: "弘援",
		old_sbhongyuan_info:
			"蓄力技（1/4）。你一次性获得至少两张牌时，可以消耗1点蓄力值并令至多两名角色各摸一张牌；一名其他角色一次性失去至少两张牌时，你可以消耗1点蓄力值令其摸两张牌。",
		old_sbmingzhe: "明哲",
		old_sbmingzhe_info:
			"锁定技，每轮限三次，你于回合外失去牌时，你选择一名角色：若其有蓄力技，其获得1点蓄力值；若你失去的牌中有非基本牌，其摸一张牌。",
		old_sb_zhangliao: "旧谋张辽",
		old_sb_zhangliao_prefix: "旧|谋",
		old_sbtuxi: "突袭",
		old_sbtuxi_info:
			"你的回合限三次，当你不因此技能获得牌后，你可以将其中任意张牌置入弃牌堆，然后获得至多X名其他角色各一张手牌（X为你以此法置入弃牌堆的牌数）。",
		old_sbdengfeng: "登锋",
		...
		old_sbwushuang: "无双",
		old_sbwushuang_info:
			"锁定技，你使用的【杀】需两张【闪】才能抵消；与你进行【决斗】的角色每次需要打出两张【杀】。此【杀】或【决斗】造成伤害时，若受伤角色没有使用或打出过【杀】或【闪】响应，此伤害+1。",
		old_sbliyu: "利驭",
		...
		old_pothanzhan: "酣战",
		old_pothanzhan_info:
			"出牌阶段限一次，你可以选择一名其他角色，你与其将手牌数摸至X张（X为各自体力上限且至多摸三张），然后你视为对其使用一张【决斗】。",
		old_potzhanlie: "战烈",
		old_potzhanlie_info:
			"①一名角色的回合开始时，你记录X（X为你的攻击范围），本回合中的前X张【杀】进入弃牌堆后，你获得等量“烈”标记（你至多拥有8个“烈”标记）。②出牌阶段结束时，你可移除全部”烈”标记（没有标记也可发动），视为使用一张无次数限制的【杀】并选择以下选项中的至多Y项（Y为你本次移除的标记数/2，向下取整）：1.令此【杀】可以额外指定一个目标；2.令此【杀】基础伤害值+1；3.令此【杀】需额外弃置一张牌方可响应；4.此【杀】结算完毕后，你摸两张牌。",
		old_potzhenfeng: "振锋",
		old_potzhenfeng_info:
			"限定技，出牌阶段，你可以选择一项：①回复2点体力；②修改〖酣战〗和〖战烈〗描述中的“X”为当前体力值、已损失体力值、场上人数中的一项（拥有对应技能方可选择）。",
		old_pot_lougui: "旧势娄圭",
		old_pot_lougui_prefix: "旧|势",
		old_potjiyu: "急御",
		old_potjiyu_info:
			"①出牌阶段限一次，你可以弃置一张手牌，从牌堆中随机获得与此牌类别不同的牌各一张。②当你失去本阶段因〖急御①〗获得的所有牌后，你重置〖急御①〗。",
		old_pot_chendao: "旧势陈到",
		old_pot_chendao_prefix: "旧|势",
		old_potwanglie: "往烈",
		old_potwanglie_info:
			"出牌阶段开始时，你可以选择一张手牌，你此阶段使用此牌无距离限制且不可被响应，且你使用此牌结算结束后，你于此阶段不能对其他角色使用牌。",
		old_pothongyi: "弘毅",
		old_pothongyi_info:
			"锁定技。①游戏开始时，你获得2个「毅」标记；当你造成或受到1点伤害后，你获得1个「毅」标记；你至多拥有4个「毅」标记。②准备阶段，你选择一项：1.摸X张牌（X为你拥有的「毅」标记数）；2.移去所有「毅」标记，视为使用等量的【杀】。",
		old_pot_yuji: "旧势于吉",
		old_pot_yuji_prefix: "旧|势",
		old_potdaozhuan: "道转",
		old_potdaozhuan_info:
			"每轮每种牌名限一次，你可以将你与当前回合角色的共X张牌置入弃牌堆（X为本回合所有角色使用牌的类别数，X为0则跳过此步骤），视为使用一张基本牌。若X不为0且当前回合角色因此失去了X张牌，则本轮此技能失效。",
		old_potfuji: "符济",
		old_potfuji_info:
			"出牌阶段限一次，你可以展示全场共计Y张手牌并令等量角色获得之（Y为场上其他角色数）。因此获得【杀】的角色使用【杀】造成的伤害+1直到你的下个回合开始；因此获得【闪】的角色使用【闪】结算完毕后摸一张牌直到你的下个回合开始。然后若你的手牌数为全场最低，则你摸两张牌，获得这两项效果直到你的下个回合开始。",
		old_pot_weiyan: "旧势魏延",
		old_pot_weiyan_prefix: "旧|势",
		old_potzhongao: "忠傲",
		old_potzhongao_info:
			"使命技，①游戏开始时，你获得〖狂骨〗。②成功：你杀死一名角色，升级〖狂骨〗，回复X点体力并摸X张牌，体力值已满则改为摸等量张牌（X为你剩余的“壮誓”次数）。③失败：你进入濒死，或你未于出牌阶段开始时执行〖壮誓〗，失去〖壮誓〗并获得〖困奋〗，然后你回复1点体力并摸两张牌。",
		old_potzhuangshi: "壮誓",
		old_potzhuangshi_info:
			"出牌阶段开始时，你可以执行任意项：1.失去任意点体力，令你此阶段使用的前等量张牌不计入次数限制；2.弃置任意张手牌，令你此阶段使用的前等量张牌无距离限制且不可被响应。若均不执行，你增加1点体力上限并回复1点体力。",
		old_potzhuangshi_tag: "已选择弃置",
		old_mb_chenzhi: "旧势陈祗",
		old_mb_chenzhi_prefix: "旧|势",
		old_mbrenxing: "任行",
		old_mbrenxing_info:
			"每回合首次有牌不于弃牌阶段被弃置时，你可选择一项：1.与当前回合角色各摸一张牌；2.弃置一名本回合未使用或打出过【杀】的角色一张牌。",
		old_guoyuan: "旧势国渊",
		old_guoyuan_prefix: "旧|势",
		old_mbxiugeng: "修耕",
		old_mbxiugeng_info:
			"回合开始时，你可以记录至多三名角色的手牌数。若如此做，这些角色的下一个摸牌阶段开始时若其手牌数：小于等于记录值，其摸两张牌；大于等于记录值，其手牌上限+1。",
		old_mbchenshe: "陈赦",
		old_mbchenshe_info:
			"当一名其他角色进入濒死状态时，你可以依次弃置你、该角色、伤害来源的各一张牌，若这些角色以此法弃置了共计三张牌，且这些牌的颜色皆相同，则其回复体力至上限，然后你失去此技能。",
		...
		old_pothaoshi: "好施",
		old_pothaoshi_info:
			"结束阶段，你可以选择一名其他角色：直到你的下个回合开始，其可以如手牌般使用或打出你的手牌。你因此失去最后的手牌时，你将手牌摸至等同于体力上限数张。",
		...
		old_mbjili: "积戾",
		old_mbjili_info:
			"一名其他角色的回合开始时，若其在你的攻击范围内，你可以从0~3中秘密选择一个本轮未选择过的数字X。若如此做，本回合结束阶段，若其本回合使用牌指定你为目标的次数：小于X，你摸4-X张牌；等于X，你交给其X张牌；大于X，你可以视为对其使用一张无距离限制的【杀】。",
		...
		old_sbbeifa: "北伐",
		old_sbbeifa_info:
			"蓄力技（3/9）。①出牌阶段，你可以消耗任意点蓄力值，将一张手牌当做等字数且未被记录的仅能对其他角色使用的牌使用或打出，然后记录之。②你于回合内受到伤害或〖北伐〗记录牌名数达到X时，清除所有记录，然后若X大于1，蓄力值上限-1（X为此技能提供的蓄力值上限）。③你的回合内，有转化牌被使用或打出时或有牌被弃置时，你获得对应实体牌数点蓄力值。",
		old_mb_zhangyan: "旧势张燕",
		old_mb_zhangyan_prefix: "旧|势",
		old_mbfeijing: "飞径",
		old_mbfeijing_info:
			"你可以将一张伤害类锦囊牌当做【杀】使用或打出。你使用【杀】指定唯一目标时，可以令你与其中间逆时针或顺时针方向上的所有角色同时展示并依次弃置一张手牌，然后你可以选择一种颜色，令弃置此颜色牌的角色成为此【杀】的额外目标，",
		old_mbxiaoge: "骁戈",
		old_mbxiaoge_info:
			"锁定技，你使用的【杀】：对因〖飞径〗成为此【杀】目标的角色造成伤害时，防止之，然后你回复1点体力并获得其因〖飞径〗弃置的牌；仅指定了一名角色为目标，此【杀】结算后你视为对其使用一张【决斗】。",
		old_mb_cuilingyi: "旧手杀崔令仪",
		old_mb_cuilingyi_ab: "旧手杀崔芙",
		old_mb_cuilingyi_prefix: "旧|手杀",
		old_mbcaiqiu: "裁裘",
		old_mbcaiqiu_info:
			"①每轮开始时，你观看牌堆顶X张牌（X为游戏人数），然后可以获得其中任意张牌。②其他角色使用牌结算结束后，若你本轮因〖裁裘〗获得过同名牌，你失去1点体力。",
		old_mbxishang: "袭裳",
		...
		old_mbkuangxiang: "匡襄",
		old_mbkuangxiang_info:
			"出牌阶段限一次，你可以与一名手牌数不大于你的其他角色交换手牌，且直到你的下个出牌阶段开始前，你或其失去所有因此获得的手牌后，你可以执行一次〖蓄业〗的效果（不受发动次数影响）。",
		old_mb_liuhui: "旧数刘徽",
		...
		old_mbchongcha: "重差",
		old_mbchongcha_info:
			"①你的点数大于等于10的牌不计入手牌上限，且使用时视为满足〖割圆〗中的点数0。②出牌阶段限一次，你可以弃置一张牌，令〖割圆〗中的X改为后移一位的值。",
		...
		old_mb_shehu: "射虎",
		old_mb_shehu_info:
			"准备阶段，你可以令一名角色装备或升级【雕弓】。出牌阶段限一次，你可以视为对一名其他角色使用X张【杀】。（X为【雕引】的升级次数）",
		old_pot_dengai: "旧势邓艾",
		...
		oldx_clanlieshi: "烈誓",
		oldx_clanlieshi_info:
			"出牌阶段，你可以执行其中一项：『受到1点火焰伤害并废除判定区；弃置手牌中的所有【闪】；弃置手牌中的所有【杀】』，然后你令一名其他角色选择执行另一项（不能选择无法执行的选项）。",
		...
		old_clanyuzhi: "迂志",
		old_clanyuzhi_info:
			"锁定技。①每轮开始时，你展示一张手牌，然后摸X张牌（X为此牌牌名字数）。②每轮结束时，若你本轮使用的牌数或你上一轮因〖迂志〗摸的牌数小于你本轮因〖迂志〗摸的牌数，你失去1点体力或失去〖保族〗。",
		old_clanxieshu: "挟术",
		...
		old_fangzhen: "放赈",
		old_fangzhen_info:
			"出牌阶段限一次，你可以横置一名角色，将手牌数摸至X张并交给其任意张牌，然后若本次为你第X次发动此技能，你翻面（X为其座位号）。",
		old_liuju: "留驹",
		old_liuju_info:
			"出牌阶段限一次，你可以与一名角色A拼点并使用拼点牌中的非基本牌。然后若你没赢或你与A相互之间的距离发生了变化，你复原武将牌或武将牌上的一个技能。",
		old_clan_hanrong: "旧族韩融",
		old_clan_hanrong_prefix: "旧|族",
		old_lianhe: "连和",
		old_lianhe_info:
			"出牌阶段限一次，你可以横置两名未被横置的角色并令其同时选择一项：1，令你摸一张牌；2，令你获得其一张牌并翻面。其下个出牌阶段结束时，其重复进行上述选择，直至其本次选择的选项序号总和大于X（X为其此阶段内获得牌的总数，且至多为5）。",
		old_huanjia: "缓颊",
		...
		old_clanyangji: "佯疾",
		old_clanyangji_info:
			"准备阶段，或你体力值变化过的回合结束时，你可以展示所有手牌，然后依次使用其中的黑色牌，直到你以此法无法使用或造成伤害。然后若以此法使用的最后一张为黑桃牌，你将之作为【乐不思蜀】置于当前回合角色的判定区。",
		old_clandandao: "耽道",
		...
		old_clanjiewu: "捷悟",
		old_clanjiewu_info:
			"出牌阶段开始时，你可以令一名角色的手牌此阶段始终对你可见。然后你此阶段使用牌指定目标后，你可以展示「捷悟」角色一张手牌，若：两张牌花色相同，你摸一张牌，若此牌本回合以此法展示过的次数大于1，你将你与其之中手牌较多的角色一张牌置于牌堆顶（若该角色为你，则改为将你的一张牌置于牌堆顶）。",
		old_clangaoshi: "高视",
		old_clangaoshi_info:
			"结束阶段，你可以亮出牌堆顶X张牌（X为本回合你发动〖捷悟〗的次数），然后你可以使用其中任意张你本回合出牌阶段未使用过的牌名的牌，若你因此使用了所有亮出牌，你摸两张牌。",
		...
		old_clanshengmo: "剩墨",
		old_clanshengmo_info:
			"当你需要使用一张未以此法使用过的基本牌时，你可以获得一张于本回合进入弃牌堆且点数不为这些牌中最大且不为这些牌中最小的牌，视为你使用需要使用的牌。",
		old_clan_xunyu: "旧族荀彧",
		old_clan_xunyu_prefix: "旧|族",
		old_clandingan: "定安",
		old_clandingan_info:
			"锁定技，你使用牌后，若本回合此前有角色使用过同名牌，你与不为此牌目标的任意其他角色各摸一张牌，然后令因此摸牌的其他角色中手牌最多的角色执行一项：1.受到你造成的1点伤害；2.随机弃置手牌中最多的同名牌。",
		old_clanfuning: "抚宁",
		old_clanfuning_info:
			"每回合你的体力值首次变化后，可以将至少X张牌交给一名其他角色（X为你已损失体力值且至少为1），若你交出的牌：颜色均相同，你回复1点体力；数量大于本回合受到过伤害的角色数，你将手牌调整至体力上限。",
		old_clan_xunshi: "旧族荀莳",
		old_clan_xunshi_prefix: "旧|族",
		old_clanqingjue: "清绝",
		old_clanqingjue_info:
			"锁定技，你手牌中每个花色仅一张的牌不计入手牌上限。当你每回合体力值首次变化后，你弃置手牌中任意张花色数量不为一的牌，并执行以下等量项：1.将这些牌交给一名其他角色；2.获得手牌中未拥有花色的牌各一张。",
		old_clanxsyingxiang: "萦香",
		old_clanxsyingxiang_info:
			"锁定技，当其他角色获得你的牌后，将此牌称为“萦香”牌。当有角色失去“萦香”牌后，你和手牌中有“萦香”牌的角色各摸一张牌。若不因使用而失去，你发动一次〖清绝〗（每轮限一次）。",
		old_zhanghua: "旧张华",
		...
		old_luochong: "落宠",
		old_luochong_info:
			"准备阶段或当你受到伤害后，你可以选择一项：1、令一名角色回复1点体力；2、令一名角色失去1点体力；3、弃置至多两名角色场上各一张牌；4、摸两张牌并可以将其交给其他角色（每轮每项各限一次）。",
		old_aichen: "哀尘",
		...
		old_dzkanpo: "勘破",
		old_dzkanpo_info:
			"出牌阶段限X次（X为你的体力值），当你使用非装备牌指定一名角色为目标后，若该角色有手牌，你可以观看其手牌并选择一项：<br>1.弃置其一张牌，然后若弃置的牌是能造成火焰伤害的牌，你摸一张牌。<br>2.重铸其手牌中的所有【杀】和【决斗】。<br>3.若其没有【闪】，你与其互相对对方造成1点伤害。",
		old_dzgengzhan: "更战",
		old_dzgengzhan_info:
			"①每当你受到1点伤害后，你可以令一名手牌数小于体力上限的角色摸三张牌，然后其将手牌弃至其体力上限。<br>②当你进入濒死状态时，你可以令一名其他角色观看随机五张魏势力武将牌，然后该角色可以从中选择一张并替换其原武将牌（体力上限与体力值不变）。",
		old_wangyan: "旧王衍",
		old_wangyan_prefix: "旧",
		old_cihuang: "雌黄",
		old_cihuang_info:
			"当有牌被抵消后，若此牌的目标数为1且此牌的使用者A为当前回合角色，则你可以视为对A使用一张你本轮内未以此法使用过的任意属性的【杀】或单目标锦囊牌（无距离限制且不可被响应）。",
		old_sanku: "三窟",
		...
		old_olcangxin: "藏心",
		old_olcangxin_info:
			"锁定技。①当你受到伤害时，你观看牌堆底的三张牌并弃置其中任意张牌，若你因此弃置了红桃牌，你防止此伤害。②摸牌阶段开始时，你展示牌堆底的三张牌，然后摸X张牌(X为其中红桃牌的数量)。",
		...
		old_hezhong: "和衷",
		old_hezhong_info:
			"每回合每项限一次，当你的手牌数变为1后，你可以展示此唯一手牌A并摸一张牌，然后你选择一项：①本回合使用点数大于A的点数的普通锦囊牌额外结算一次；②本回合使用点数小于A的点数的普通锦囊牌额外结算一次。",
		old_ol_pengyang: "旧彭羕",
		...
		old_olzhiyan: "直言",
		old_olzhiyan_info:
			"你或你的上家的结束阶段，你可以令一名角色正面朝上摸一张牌，然后若此牌：为装备牌，则其使用此牌并回复1点体力；不为装备牌且其体力值不小于你，则其失去1点体力。",
		old_zhugejin: "旧诸葛瑾",
		...
		old_qiaoli: "巧力",
		old_qiaoli_info:
			"出牌阶段，你可以将一张装备牌当作【决斗】使用。若此牌：为武器，此牌结算后你摸等同于其攻击范围的牌，并可以将这些牌分配给任意角色；不为武器，此【决斗】不可被响应。",
		old_qiaoli_given: "已分配",
		old_qingliang: "清靓",
		old_qingliang_info:
			"每回合限一次，当你成为其他角色使用的基本牌或普通锦囊牌的目标时，你可以展示所有手牌，然后选择一项：1，与其各摸一张牌；2，弃置一种花色的所有手牌，令此牌对你无效。",
		old_xurong: "旧徐荣",
		old_xurong_prefix: "旧",
		old_xionghuo: "凶镬",
		old_xionghuo_info:
			"游戏开始时，你获得3个“暴戾”标记。出牌阶段，你可以交给一名其他角色一个“暴戾”标记。当你对有“暴戾”标记的其他角色造成伤害时，此伤害+1。有“暴戾”标记的其他角色的出牌阶段开始时，其移去所有“暴戾”标记并随机执行一项：1.受到1点火焰伤害且本回合不能对你使用【杀】；2.失去1点体力且本回合手牌上限-1；3.你随机获得其一张手牌和一张装备区里的牌。",
		old_shajue: "杀绝",
		...
		old_olguanbian: "观变",
		old_olguanbian_info:
			"锁定技。①游戏开始时，你的手牌上限、其他角色计算与你的距离、你计算与其他角色的距离+X（X为游戏人数）。②第二轮游戏开始时，或当你发动〖凶逆〗或〖封赏〗后，你失去〖观变〗。",
		old_olxiongni: "凶逆",
		old_olxiongni_info: "出牌阶段开始时，你可以弃置一张牌，然后所有其他角色选择一项：1.弃置一张与此牌花色相同的牌；2.受到你的1点伤害。",
		old_olfengshang: "封赏",
		old_olfengshang_info:
			"出牌阶段限一次或当一名角色进入濒死状态时，你可以将两张本回合进入弃牌堆中的花色相同的牌分配给等量角色。若你未以此法获得牌，你视为使用一张不计入次数的【酒】。",
		old_olzhibin: "执柄",
		...
		old_olhedao_faq: "“天书”",
		old_olhedao_faq_info:
			"关于“天书”：<br>" +
			[
				"“天书”为随机三个时机和三个效果中各选择一个组成的技能，且时机等级越高，效果等级越高",
		...
		old_olsbjigu: "积谷",
		old_olsbjigu_info:
			"锁定技。一名角色于其出牌阶段外使用的牌进入弃牌堆后，若“谷”的数量小于你的体力上限，你将其中的非♥牌置于你的武将牌上，称为“谷”。体力上限与你相同的角色的回合开始时，你用任意张手牌替换等量“谷”。",
		old_olsbjiewan: "解腕",
		old_olsbjiewan_info:
			"①每个准备阶段，你可以减少1点体力上限或移去两张“谷”，然后将一张手牌当无距离限制的【顺手牵羊】使用。②每个结束阶段，若你的手牌数与“谷”的数量相同且你的体力上限不为全场唯一最高，你增加1点体力上限。",
		old_ol_wangyi: "旧OL界王异",
		old_ol_wangyi_prefix: "旧|OL界",
		old_olzhenlie: "贞烈",
		old_olzhenlie_info:
			"当你成为其他角色使用的【杀】或非延时锦囊牌的目标后，你可以失去一点体力令此牌对你无效，然后选择一项：1，获得使用者的一张牌；2，发动一次【秘计】。",
		old_ol_chengpu: "旧OL界程普",
		old_ol_chengpu_prefix: "旧|OL界",
		old_olchunlao: "醇醪",
		old_olchunlao_info:
			"①当你或你的上下家的【杀】因弃置进入弃牌堆后，你将位于弃牌堆的这些牌称为“醇”置于武将牌上。②一名角色处于濒死状态时，你可以将一张“醇”置入弃牌堆，然后令其视为使用一张【酒】。③当一名角色失去体力后，你可以获得至多两张“醇”。",
		old_ol_xuelingyun: "旧OL薛灵芸",
		old_ol_xuelingyun_prefix: "旧|OL",
		old_olsiqi: "思泣",
		old_olsiqi_info:
			"你的红色牌进入弃牌堆时，将之置于牌堆底。当你受到伤害后，你可以亮出牌堆底至多五张连续的红色牌，使用其中【桃】、【无中生有】与装备牌（可指定其他角色为目标），然后你摸剩余不可使用的牌数张牌。",
		...
		oldx_oljiawei: "假威",
		oldx_oljiawei_info:
			"出牌阶段限一次，你可以选择一名角色，弃置其X张牌，然后亮出牌堆顶X+1张牌，你获得其中的伤害牌并令其获得其中的非伤害牌。若如此做，本回合你使用牌无距离限制（X为你执行过的回合数且至多为你的体力上限）。",
		old_olqujia: "驱驾",
		old_olqujia_info:
			"锁定技，回合结束后，若你本回合未杀死角色，你选择一项：1.执行一个额外回合，若此回合内你未杀死角色，你失去此技能并摸X张牌；2.令你本局游戏使用【杀】的次数+1并获得一张【杀】（X为你执行过的回合数且至多为你的体力上限）。",
		...
		old_dcporui: "破锐",
		old_dcporui_info:
			"每轮限一次，其他角色的结束阶段，你可以弃置一张基本牌并选择另一名此回合内失去过牌的其他角色，你视为对该角色依次使用X+1张【杀】，然后你交给其X张手牌。（X为你的体力值，手牌不足X张则全给）",
		old_dcgonghu: "共护",
		old_dcgonghu_info:
			"锁定技，当你于回合外失去基本牌后，〖破锐〗最后增加描述“若其没有因此受到伤害，你回复1点体力”；当你于回合外造成或受到伤害后，你删除〖破锐〗中“交给”效果的描述。若以上两个效果均已触发，则你本局游戏接下来你使用红色基本牌无法响应，使用红色普通锦囊牌可以额外指定一个目标。",
		old_ruanyu: "旧阮瑀",
		old_ruanyu_prefix: "旧",
		old_miaoxian: "妙弦",
		old_miaoxian_info:
		...
		old_dclinghui: "灵慧",
		old_dclinghui_info:
			"每名角色的结束阶段，若本回合有至少两名角色受到过伤害，你可以观看的牌堆顶的三张牌：你可以使用其中一张牌，然后获得其余的牌。",
		oldx_dclinghui: "灵慧",
		oldx_dclinghui_info:
			"一名角色的结束阶段，若当前回合角色为你或本回合有角色进入过濒死状态，则你可以观看牌堆顶的三张牌并使用其中一张牌，然后获得剩余的牌。",
		old_dcyuxin: "御心",
		old_dcyuxin_info:
			"限定技，一名角色进入濒死状态时，你可以令其回复体力至1点。若以此法回复体力的是其他角色，你与其体力值互换且你摸等同于你以此法失去的体力值张牌。",
		old_dc_liuli: "旧刘理",
		old_dc_liuli_prefix: "旧",
		old_dcfuli: "抚黎",
		old_dcfuli_info:
			"出牌阶段，你可以展示手牌并弃置一种类别的所有手牌（每种类别每回合限一次），然后摸X张牌（X为这些牌的牌名字数和且X至多为场上手牌数最多的角色的手牌数）。若你因此弃置了伤害类卡牌，则你可以选择一名角色，令其攻击范围-1直到你的下个回合开始。",
		old_dcdehua: "德化",
		old_dcdehua_info:
		...
		old_dcjigu: "激鼓",
		old_dcjigu_info:
			"锁定技。①游戏开始时，你将所有手牌标记为“激鼓”。②你的“激鼓”牌不计入手牌上限。③当你造成或受到伤害后，若你的“激鼓”牌数等于你的装备区牌数，则你可以摸X张牌（X为你的空置装备栏数）。",
		old_wu_zhugeliang: "旧武诸葛亮",
		old_wu_zhugeliang_prefix: "旧|武",
		old_dcqingshi: "情势",
		old_dcqingshi_info:
			"当你于出牌阶段使用牌时，若你手牌中有同名牌，你可以选择一项：1.令此牌对其中一个目标角色造成的伤害+1；2.令任意名其他角色各摸一张牌；3.摸X张牌，然后【情势】于本回合失效（X为你的体力值）。",
		old_dczhizhe: "智哲",
		old_dczhizhe_clear: "invisible",
		old_dczhizhe_info:
			"限定技。出牌阶段，你可以选择一张手牌并复制之。该复制牌不计入你的手牌上限，且当你使用或打出此牌结算结束后，你获得之，然后你本回合不能再使用或打出此牌。",
		old_wu_guanyu: "旧武关羽",
		old_wu_guanyu_prefix: "旧|武",
		old_dcjuewu: "绝武",
		old_dcjuewu_info:
			"①你可以将一张点数为2的牌当任意伤害类牌使用（包括【水淹七军】）。②当你得到其他角色区域内的牌后，你令这些牌的点数均视为2直到你失去这些牌。",
		old_dcwuyou: "灵显",
		old_dcwuyou_info:
			"其他角色的出牌阶段限一次，其可以交给你一张手牌，然后你可以声明一个非装备牌的牌名（包括【水淹七军】）并交给其一张手牌，令此牌的牌名与属性视为与你选择的相同。",
		old_dcyixian: "义贤",
		old_dcyixian_info:
			"限定技。出牌阶段，你可以获得场上和其他角色手牌中的所有武器和防具牌。然后你依次选择是否令被你以此法获得牌的角色摸X张牌或回复1点体力（X为其以此法失去的牌数）。",
		...
		old_dclianjie: "连捷",
		old_dclianjie_info:
			"你使用手牌指定目标后，若此牌点数不大于你的所有手牌，你可令一名角色将其一张点数最小的手牌置于牌堆底，然后你将手牌摸至体力上限，以此法获得的牌本回合无距离次数限制（每个点数每回合限摸一次，无点数视为0）。",
		old_dcjiangxian: "将贤",
		old_dcjiangxian_info:
			"限定技，出牌阶段，你可以获得以下效果直到本回合结束：当你使用因〖连捷〗获得的牌造成伤害时，此伤害+X（X为你本回合造成伤害的次数且至多为5）。若如此做，本回合结束后你失去〖连捷〗或〖朝镇〗。",
		...
		old_gouzhu: "苟渚",
		old_gouzhu_info:
			"你发动技能后，若此技能为：锁定技，回复1点体力；觉醒技，获得一张基本牌；限定技，对随机一名其他角色造成1点伤害；转换技，手牌上限+1；主公技，增加1点体力上限。",
		old_zhugejing: "旧诸葛京",
		...
		old_xinquanji: "权计",
		old_xinquanji_info:
			"①当你受到1点伤害后，或其他角色不因你的赠予而得到你的牌后，你可以摸一张牌，然后将一张手牌置于武将牌上，称为“权”。②你的手牌上限+X（X为“权”的数量）。",
		old_xinzili: "自立",
		...
		old_stardangchen: "荡尘",
		old_stardangchen_info:
			"出牌阶段开始时，你可以令一名角色交给你任意张牌，然后你本回合使用基本牌或普通锦囊牌时，可以进行一次判定，若判定的点数为其他角色此次其交给你牌的数量的倍数，则此牌额外结算一次。",
		old_pangfengyi: "旧庞凤衣",
		old_pangfengyi_prefix: "旧",
		old_dcyitong: "异瞳",
		old_dcyitong_info:
			"锁定技。①游戏开始时，你记录一个花色。②每回合〖异瞳〗记录花色的牌首次进入弃牌堆后，你从牌堆或弃牌堆获得与此花色不同的牌各一张。",
		old_dcpeiniang: "醅酿",
		old_dcpeiniang_info:
			"①你可以将〖异瞳〗记录花色的牌当作【酒】使用（无任何次数限制）。②其他角色处于濒死状态时，你可以对其使用【酒】（回复效果）。③因你使用【酒】执行的体力回复效果至少令目标角色将体力值回复至1点。",
		old_dc_sp_zhurong: "旧SP祝融",
		old_dc_sp_zhurong_prefix: "旧|SP",
		old_dcremanhou: "蛮后",
		old_dcremanhou_info:
			"出牌阶段限一次，你可以摸至多四张牌并根据摸牌数依次执行以下等量项：①失去〖探乱〗；②弃置一张手牌；③失去1点体力，获得一名其他角色的一张手牌；④弃置场上的一张牌，获得〖探乱〗。",
		old_dcretanluan: "探乱",
		...
		old_dcwoheng: "斡衡",
		old_dcwoheng_info:
			"出牌阶段或当你受到伤害后，你可令一名角色摸或弃置X张牌，选择其他角色结算后其手牌数与你不同或选择自己时，此技能失效至本回合结束(X为此技能本轮发动的次数)。",
		old_dcjizheng: "集征",
		old_dcjizheng_info:
			"威主技，其他吴势力角色出牌阶段限一次，其可交给你一张牌，则其本回合使用牌无距离限制。其他角色出牌阶段限一次，其可交给你一张牌，则其本回合使用的下张牌无距离限制。",
		...
		old_dcsbshimou: "势谋",
		old_dcsbshimou_info:
			"转换技，游戏开始可自选阴阳状态，出牌阶段限一次，你可令一名{阳：手牌数全场最低的角色；阴：手牌数全场最高的角色}将手牌调整至体力上限（至多摸五张）并视为使用一张仅指定单目标的普通锦囊牌（此牌牌名与目标由你指定）。若以此法摸牌，此牌可额外增加一个目标；若以此法弃牌，此牌额外结算一次。",
		old_dc_xiahouxuan: "旧新杀夏侯玄",
		old_dc_xiahouxuan_prefix: "旧|新杀",
		old_dcyizheng: "议政",
		old_dcyizheng_info:
			"你的回合开始和结束时，你可与任意名其他角色各展示一张手牌，若展示的牌类型均相同，你可将这些牌交给一名角色，否则，你弃置这些牌。",
		old_dcguilin: "归林",
		...
		old_dcgumai: "孤脉",
		old_dcgumai_info:
			"每回合限一次，你造成或受到伤害时，若你有手牌，你可以展示所有手牌并令此伤害+1/-1。若展示的牌花色均相同，你可以弃置一张手牌令此技能视为未发动过。",
		old_dc_tengfanglan: "旧滕芳兰",
		old_dc_tengfanglan_prefix: "旧",
		old_dcaichen: "哀尘",
		old_dcaichen_info:
			"锁定技。若牌堆剩余数：大于80，当你每次发动〖落宠〗弃置你区域内的牌后，你摸两张牌；大于40，你跳过弃牌阶段；小于40，你不能响应♠牌。",
		old_dc_sb_dengai: "旧新杀谋邓艾",
		old_dc_sb_dengai_prefix: "旧|新杀谋",
		old_dcsbzhouxi: "骤袭",
		old_dcsbzhouxi_info:
			"出牌阶段限一次，你弃置所有当前无法指定其他角色为目标的手牌，并根据弃牌数依次选择等量项：1.本回合计算与其他角色距离-X；2.视为对至多X名角色使用【顺手牵羊】；3.视为对至多X名角色使用【杀】。若你执行了所有选项，此技能视为未发动过（X未此项被选择的次序）。",
		old_dcsbshijin: "恃矜",
		old_dcsbshijin_info:
			"限定技，出牌阶段，若你本回合造成过伤害，你可以获得每种类型的牌各一张，且直到你的下个回合开始前，你受到伤害时防止之并摸一张牌。你的下个回合开始时，弃置所有【杀】和锦囊牌并失去等量体力，若你未因此失去体力，此技能视为未发动过。",
		old_renwan: "旧任婉",
		old_renwan_prefix: "旧",
		old_dcjuanji: "狷急",
		old_dcjuanji_info:
			"摸牌阶段开始时，你可以摸体力上限张牌；出牌阶段开始时，你可以失去1点体力，然后视为对一名角色使用一张【杀】；弃牌阶段开始时，你可以调整手牌至手牌上限，然后弃置一名角色区域里至多两张牌。",
		old_dcrenshuang: "纫霜",
		old_dcrenshuang_info:
			"锁定技，①你每轮首次进入濒死时，回复体力至1点并增加1点体力上限（至多以此法增加3点）。②你脱离濒死时，复原武将牌并视为使用一张普通锦囊牌。",
		old_lukai: "旧陆凯",
		old_lukai_prefix: "旧",
		old_bushi: "卜筮",
		old_bushi_info:
			"①你使用♠牌无次数限制；当你使用或打出♥牌后，你摸两张牌；结束阶段，或当你成为♣牌的目标后，你从牌堆或弃牌堆获得一张♦牌。②准备阶段，你可调整此技能中四种花色的对应顺序。",
		old_zhongzhuang: "忠壮",
		...
		old_dczhongtao: "众讨",
		old_dczhongtao_info:
			"①出牌阶段限一次，你可以选择至多X+2种花色（X为你已损失的体力值），然后随机获得场上、弃牌堆或牌堆中你选择花色的各一张牌。②当你于回合内使用三种类别的牌后，此技能视为未发动过。",
		old_wufu: "旧伍孚",
		old_wufu_prefix: "旧",
		...
		old_dcsbchuanyu_info:
			"①每轮开始时，你可摸一张牌然后交给一名角色一张牌，称为「舆」。②每当「舆」因使用进入弃牌堆时，你可将其交给本轮未获得过「舆」的一名角色。③每轮结束时，你可令本轮所有获得过「舆」的角色依次视为对你指定的一名角色使用【杀】(不限距离），然后弃置所有「舆」。",
		old_dcsbyitou: "倚投",
		old_dcsbyitou_info:
			"其他角色的出牌阶段开始时，若其手牌数为全场最多，你可将所有手牌交给该角色，直到你的下回合开始，该角色造成伤害后，你摸一张牌。",
		old_chengui: "旧陈珪",
		old_chengui_prefix: "旧",
		old_yingtu: "营图",
		old_yingtu_info:
			"①当你的上家于摸牌阶段外获得牌后，你可以获得其等量的牌，然后将等量的牌交给你的下家。②当你的下家使用【杀】或【决斗】指定第一个目标时，若目标角色不包含你和你的上家，则你可以取消此牌的所有目标，然后将此牌目标改为你的上家。",
		old_congshi: "从势",
		old_congshi_info:
			"锁定技。①体力值最大的角色对你的上家和下家使用牌无距离限制。②有角色使用因〖从势①〗增加距离的牌对你的上家或下家造成伤害后，你回复1点体力。",
		old_dc_sb_luxun: "旧新杀谋陆逊",
		old_dc_sb_luxun_prefix: "旧|新杀谋",
		old_dcsbjunmou: "隽谋",
		old_dcsbjunmou_info:
			"转换技。①游戏开始时，你可以转换此技能状态；②一张牌结算结束后，若此牌的目标包括你，你可以摸一张牌并选择一张手牌，阳：此牌视为无次数限制的火【杀】；阴：重铸此牌并横置一名角色。",
		old_dcsbzhanyan: "绽炎",
		old_dcsbzhanyan_info:
			"限定技，出牌阶段，你可选择任意名横置的其他角色并摸等量张牌，然后这些角色同时展示一张手牌，你可弃置相同花色牌并对展示对应花色的角色各造成1点火焰伤害，若这些角色均受到伤害则重复此流程， 且此技能结算期间你每失去一张牌则摸一张牌。",
		old_v_sunce: "旧威孙策",
		old_v_sunce_prefix: "旧|威",
		old_dczhifeng: "猘锋",
		old_dczhifeng_info:
			"每回合限X次（X为场上人数）。当你的手牌数：大于体力值时，你可将至少两张黑色牌当作不计入次数的【酒】使用；小于体力值时，你可将一张红色牌当作【杀】或【闪】使用或打出，然后将手牌摸至体力上限；等于体力值时，你可将任意张牌当作【决斗】使用。",
		old_dcweijing: "威靖",
		...
		old_dccangming: "沧溟",
		old_dccangming_info:
			"锁定技，分发初始手牌后，你令所有角色将手牌置于武将牌上，称为“溟”。有牌进入“溟”时，每包含一个类别，你摸一张牌。一名角色受到伤害后或回合开始时，获得其武将牌上的所有“溟”。",
		old_dcchouxi: "筹汐",
		old_dcchouxi_info: "出牌阶段，你可将一张牌当作“溟”中的一张基本牌或普通锦囊牌使用（每种牌名每回合限一次），以此法使用牌无距离次数限制。",
		old_dcjichao: "激潮",
		old_dcjichao_info:
			"出牌阶段限一次，你可选择一项：1.令一名其他角色将随机一半数量的手牌（向上取整）和装备区的牌置于武将牌上，称为“溟”；2.令所有其他角色将所有牌置于武将牌上，称为“溟”，然后此选项失效直到你累计造成3点伤害。",
		old_hansong: "旧韩嵩",
		...
		old_twmutao: "募讨",
		old_twmutao_info:
			"出牌阶段限一次。你可以选择一名角色，令其将手牌中所有的【杀】依次交给其下家开始的每一名角色。然后其对最后一名以此法获得【杀】的角色A造成X点伤害（X为A手牌中【杀】的数量且至多为3）。",
		old_twyimou: "毅谋",
		...
		old_twbudao: "布道",
		old_twbudao_info:
			"限定技。准备阶段，你可减1点体力上限，回复1点体力并从【咒护】【丰祈】【阻祸】中选择一个技能获得。然后你可以令一名其他角色也获得此技能并交给你一张牌。",
		...
		old_twjuntun: "军屯",
		old_twjuntun_info:
			"①游戏开始时或当其他角色濒死状态结算完成后，你可令一名角色获得【凶军】。②当其他角色造成伤害后，若其拥有【凶军】，你获得等同于此次伤害值的暴虐值。",
		old_twxiongxi: "凶袭",
		...
		old_twchue: "除恶",
		old_twchue_info:
			"①当你使用【杀】指定唯一目标时，若场上存在可成为此【杀】目标的非目标角色，则你可以失去1点体力，为此牌额外指定Z个目标。②当你受到伤害或失去体力后，你摸一张牌并获得1个“勇”标记。③回合结束时，若你的“勇”标记数大于等于Z，则你可以失去Z个“勇”标记，视为使用一张伤害+1且可以额外指定Z个目标的【杀】。（Z为你的体力值）",
		old_huan_zhugeliang: "旧幻诸葛亮",
		old_huan_zhugeliang_prefix: "旧|幻",
		old_twhunyou: "魂游",
		old_twhunyou_info:
			"限定技，当你处于濒死状态时，你可以将体力值回复至1点，若如此做，本回合当你受到伤害时或失去体力时，取消之；当前回合结束后，你入幻：摸X张牌并进行一个额外的回合（X为〖北定〗记录牌名数，且至多为7）。",
		old_twchanggui: "怅归",
		...
		old_twchihui: "炽灰",
		old_twchihui_info:
			"其他角色的回合开始时，你可废除一个装备栏并选择一项:1.弃置其区域内的一张牌；2.將牌堆中的一张与此次废除的装备栏相同副类别的装备牌置入其装备区。若如此做，你失去1点体力，然后摸X张牌（X为你已损失的体力值）。",
		old_twfuxi: "赴曦",
		...
		old_twhuangzhu: "煌烛",
		old_twhuangzhu_info:
			"准备阶段，你可选择一个废除的装备栏， 从牌堆或弃牌堆中随机获得一张对应副类别的装备牌，并记录其牌名。出牌阶段开始时，你可选择或变更至多两个已记录且与已废除的装备栏相同副类别的装备牌牌名（每种副类别限一个）。你视为拥有选择的牌名的效果直到此装备栏恢复。",
		old_twliyuan: "离渊",
		old_twliyuan_info: "你可将一张与你已废除的装备栏对应副类别的装备牌当【杀】使用或打出（无距离和次数限制），然后你摸两张牌。",
		old_twjifa: "冀筏",
		old_twjifa_info:
			"锁定技，当你进入濒死状态时，你减X点体力上限（X为你上次发动〖赴曦〗选择的项数），保留〖煌烛〗或〖离渊〗直到下次入幻， 然后退幻：将体力值回复至体力上限。",
		...
		old_rexianyuan: "仙援",
		old_rexianyuan_info:
			"①一轮游戏开始时，你获得4枚“仙援”标记（一名角色至多拥有4枚“仙援”标记）。②出牌阶段，你可以将“仙援”标记分配给其他角色。③拥有“仙援”标记的角色的出牌阶段开始时，你选择一项：⒈观看其手牌，将其中至多X张牌置于牌堆顶；⒉令其摸X张牌（X为其拥有的“仙援”标记数）。然后若当前回合角色不为你，则移去其所有“仙援”标记。",
		...
		old_twdanfa: "丹法",
		old_twdanfa_info:
			"当你使用或打出的牌结算结束后，若此牌花色与你拥有的「丹」均不相同，你可以将此牌置于你的武将牌上，称为「丹」，然后摸一张牌。",
		old_twlingbao: "灵宝",
		old_twlingbao_info:
			"出牌阶段，你可以弃置两张花色不同的「丹」并摸两张牌，然后根据其情况执行如下效果：均为红色，你令一名角色从牌堆中获得两张基本牌；均为黑色，你弃置一名角色至多两个不同区域的共计至多两张牌；颜色不同，你令一名角色摸两张牌，另一名角色弃一张牌。然后若你于本回合弃置过两张相同花色的「丹」，则此技能失效直到回合结束。",
		old_twsidao: "司道",
		...
		old_twjuxia: "居下",
		old_twjuxia_info:
			"①每回合限一次，其他角色使用牌指定你为目标后，若其技能数多于你，则你可以令此牌对你无效，然后你摸两张牌。②准备阶段，若你没有〖观火〗，则你可以获得之。",
		old_yinfuren: "旧尹夫人",
		...
		old_twjilun: "机论",
		old_twjilun_info:
			"当你受到伤害后，你可以摸X张牌（X为【急筹】已记录牌名的数量且X至少为1），或视为使用一张〖急筹①〗记录过且未被〖机论〗记录过的普通锦囊牌并记录此牌。",
		...
		lmzhiti: "止啼",
		lmzhiti_info:
			"锁定技。①你攻击范围内已受伤的其他角色手牌上限-1；②当你和已受伤的角色拼点或【决斗】胜利/受到已受伤角色造成的伤害后，若对方/伤害来源在你的攻击范围内，则你恢复一个装备栏。③若场上已受伤的角色数：不小于1，你的手牌上限+1；不小于3，你于摸牌阶段开始时令额定摸牌数+1；不小于5，回合结束时，你废除一名角色的一个随机装备栏。",
		lmjunkguixin: "归心",
		...
		wechattianfa: "天罚",
		wechattianfa_info:
			"出牌阶段，你每使用两张锦囊牌，你获得1枚“罚”标记；回合结束时，你可以对至多X名其他角色各造成1点伤害（X为你拥有的“罚”标记数）。",
		minimeihun: "魅魂",
		minimeihun_info:
			"结束阶段，或你于当前回合首次成为【杀】的目标后，你可以选择一名其他角色，然后声明一个花色，令其交给你所有你此花色的牌，若其没有此花色的牌，则你观看其手牌并获得其中一张。",
		minihuoxin: "惑心",
		minihuoxin_info:
			"出牌阶段限一次，你可以弃置一张牌并令两名角色拼点，然后你可以声明一个花色，没赢的角色须选择一项：①令你获得其所有此花色的牌；②其不能使用或打出你此次声明的花色的牌直到其下个回合结束。",
		miniqinyin: "琴音",
		miniqinyin_info:
			"弃牌阶段结束时，若你于此阶段内弃置过牌，则你可以选择一项：1. 令所有角色各回复1点体力；2. 令所有角色各失去1点体力；3.令所有角色各摸一张牌。",
